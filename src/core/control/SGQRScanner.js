...
export class SGQRScanner extends SGBaseControl {
    ...
    static _initPreset() {
        if (!SGQRScanner._isPresetInit) {
            var { w, h, p } = SGHelperWindow.getWHPNoHeader();
            SGQRScanner._presetStyle = {
                default: StyleSheet.create({
                    v1: { padding: 2 * p, paddingBottom: p, backgroundColor: SGHelperStyle.color.SGQRScanner.BGWhite, marginVertical: 2 * p, borderRadius: 2 * p },
                    icon1: { margin: 0, paddingVertical: 0 },
                ...
                disabled: StyleSheet.create({
                    v1: { padding: 2 * p, paddingBottom: p, backgroundColor: SGHelperStyle.color.SGQRScanner.BGDisabled, marginVertical: 2 * p, borderRadius: 2 * p },
                    icon1: { margin: 0, paddingVertical: 0 },
                    t1: { color: SGHelperStyle.color.SGQRScanner.TextBlack },
                    pv1: { paddingVertical: 0, paddingHorizontal: 2 * p, backgroundColor: SGHelperStyle.color.SGQRScanner.PVBGBlack, width: w, height: SGHelperWindow.getWHP().h * 0.8, borderTopLeftRadius: 5 * p, borderTopRightRadius: 5 * p, justifyContent: 'flex-start' },
                    pv1_t1: { color: SGHelperStyle.color.SGQRScanner.TextWhite },
            ...
            SGQRScanner._isPresetInit = true;
        }
    }
    constructor(props, context, ...args) {
        super(props, context, ...args);
        this.labels = labels;
        SGQRScanner._initPreset();
    ...
    render() {
        var style = SGQRScanner._presetStyle[this.props.disabled ? 'disabled' : 'default'];
        ...
        return (
            ...
            <SGTouchableOpacity disabled={this.props.disabled} onPress={this.onShowHandler.bind(this)} shadow={this.props.shadow}>
                <SGView accessible={true} accessibilityLabel={'SGQRScannerRootView'} style={style.v1} >
                    <SGIcon disabled={this.props.disabled} accessible={true} accessibilityLabel={'SGQRScannerIconButton'} preset={iconPr} name={SGIcon.Icon.qrscan} />
                    <SGText accessible={true} accessibilityLabel={'SGQRScannerText'} preset={SGText.preset.titleH3} style={style.t1}>{this.labels[this._lang].scan}</SGText>
