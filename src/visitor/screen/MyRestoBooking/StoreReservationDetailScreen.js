...
import { SGView as View, SGRootView as RootView, SGText as Text, SGIcon as Icon, SGScrollView as ScrollView, SGTouchableOpacity as TouchableOpacity, SGButton as Button, SGDialogBox, SGImage as Image, SGActivityIndicator as ActivityIndicator } from '../../../core/control';
...
export class StoreReservationDetailScreen extends SGBaseScreen {
    ...
    async componentDidMount() {
        await this._onSelectProfileStore();
    ...
    async _onSelectProfileStore() {
        ...
        this.baseAddAPIParallel('getStoreHomeProfile', (async (v1) => { return VStoreProfileAPI.getStoreHomeProfile(v1) }).bind(this, this.reservationData.fStoreKey), ((v) => {           
            this.profileStore = v
        }).bind(this), (()=>{SGDialogBox.hideDialogBox(this.dbID3, true)}).bind(this));
        
        this.baseAddAPIParallel('getStoreSettings', (async (v1) => { return VStoreHomeAPI.getStoreSettings(v1); }).bind(this,this.reservationData.fStoreKey), ((v) => {
            this.dataSetting = v;
        }).bind(this),  (()=>{SGDialogBox.hideDialogBox(this.dbID3, true)}).bind(this));
    ...
    render() {
        ...
        return (
            <RootView dummyStatusBar accessible={true} accessibilityLabel={'StoreReservationDetailScreenRootView'} style={style.mainView1}>
                 <CustomMenuBar btnBack accessible={true} accessibilityLabel={'StoreReservationHeader'} currentUser={this.currentUser} navigator={this.props.navigation} imageSetting={this.imageSetting} style={style.throwWHP}></CustomMenuBar>
                <Image accessible={true} accessibilityLabel={'ViewWaitingListScreenImage'} style={style.containerView1} source={{ uri: this.profileStore.length !== 0 ? this.profileStore['fContent' + this._language.toUpperCase()].fStoreBackgroundImageJSON[0][this.imageSetting].uri : '' }}></Image>
                ...
                <View accessible={true} accessibilityLabel={'StoreReservationDetailScreenContainerView'} style={style.containerView2_1}>
                    <View accessible={true} accessibilityLabel={'StoreReservationDetailScreenContainerView_1'} style={style.containerView2_2}>
                        <Text accessible={true} accessibilityLabel={'StoreReservationDetailScreenDateText'} preset={Text.preset.titleH3} style={style.text3}>{SGHelperType.formatDate(SGHelperType.convertNewDate(this.reservationData.fBookDateTime), this._language)}</Text>
                    </View>
                    <View accessible={true} accessibilityLabel={'StoreReservationDetailScreenContainerView_2'} style={style.containerView2_2}>
                        <Text accessible={true} accessibilityLabel={'StoreReservationDetailScreenTimeText'} preset={Text.preset.titleH3} style={style.text3}>{SGHelperType.formatTime(SGHelperType.convertNewDate(this.reservationData.fBookDateTime), this._language)}</Text>
                    </View>
                </View>
                ...
                {this.doneRenderProfileStore ?
                <ScrollView style={{ marginBottom: SGHelperWindow.getHeaderHeight()}} accessible={true} accessibilityLabel={'StoreReservationDetailScreenScrollView'} contentContainerStyle={{ bacgroundColor: 'white' }}>
                    <ReservationForm accessible={true} accessibilityLabel={'StoreReservationDetailScreenForm'} style={style.throwWHP} data={this.reservationData} userData={this.currentUserData} language={this._language} disabled={true} ></ReservationForm>
                    ...
                    {this.activeReservation === true && <Button accessible={true} accessibilityLabel={'StoreReservationDetailScreenCancelButton'} label={SGLocalize.translate('ReservationAddView.CancelButtonText')} style={style.button} onPress={this._onCancelPress.bind(this)}></Button>}
                     <View style={{width:w,height:w*0.15,backgroundColor:'transparent'}}></View>
                </ScrollView>
                :
                <ActivityIndicator preset={ActivityIndicator.preset.h1} style={{ flex: 1 }}></ActivityIndicator>
                }
                <BottomNavigationContainer accessible={true} accessibilityLabel={'StoreReservationDetailScreenBottomNav'} blackTheme navigator={this.props.navigation} style={style.containerView3}></BottomNavigationContainer>
            </RootView>
        );
    }
}
