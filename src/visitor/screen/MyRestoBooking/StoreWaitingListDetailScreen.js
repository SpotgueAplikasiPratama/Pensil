...
import { SGView as View, SGRootView as RootView, SGText as Text, SGIcon as Icon, SGScrollView as ScrollView, SGTouchableOpacity as TouchableOpacity, SGActivityIndicator as ActivityIndicator, SGButton as Button, SGImage as Image, SGDialogBox } from '../../../core/control';
...
export class StoreWaitingListDetailScreen extends SGBaseScreen {
    ...
    constructor(props, context, ...args) {
        ...
        this.activeWaitingList = this.props.route.params.active;
    ...
    async _onSelectProfileResto() {
        ...
        this.baseRunSingleAPIWithRedoOption('getStoreHomeProfile', (async (v1) => { return VStoreProfileAPI.getStoreHomeProfile(v1) }).bind(this, this.waitingListData.fStoreKey), ((v) => {           
            this.profileResto = v
        }).bind(this), (()=>{SGDialogBox.hideDialogBox(this.dbID3, true);}).bind(this));
        
        this.baseAddAPIParallel('getStoreSettings', (async (v1) => { return VStoreHomeAPI.getStoreSettings(v1); }).bind(this,this.waitingListData.fStoreKey), ((v) => {
            this.dataSetting = v;
        }).bind(this),  (()=>{SGDialogBox.hideDialogBox(this.dbID3, true)}).bind(this));
    ...
    async _onCancelPressHandler(){
        try {
            ...
            await tbVWaitingListAPI.cancelWaitingListVisitor(this.waitingListData)
            this.filterWaitingData = this.getFilterDataWaiting();
            this.pagingWaitingList = this.getPagingWaitingList();
            var result = await tbVWaitingListAPI.searchWaitingListMyBookingVisitor(this.filterWaitingData, [], this.pagingWaitingList)
            console.log(result)
            if(result.length == 0){
                SGHelperGlobalVar.addVar('GlobalCallWaitingList',false) 
            }
    ...
    render() {
        ...
        return (
            ...
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'RestoWaitingListDetailScreenRootView'} style={style.mainView1}>
                ...
                <View accessible={true} accessibilityLabel={'AddWaitingListScreenContainerIconView1'} style={style.containerView2}>
                    <Text preset={Text.preset.titleH1B} style={style.text1}>{this.activeWaitingList === true ? SGLocalize.translate('WaitingListAddView.TitleViewWaitingList') : SGLocalize.translate('WaitingListAddView.TitleHistoryWaitingList')}</Text>
                    <Text preset={Text.preset.titleH2B} style={style.text4}>{this.waitingListData.fStatus.toUpperCase()}</Text>
                </View>
                <View accessible={true} accessibilityLabel={'AddWaitingListScreenContainerIconView2'} style={style.containerView2_1}>
                    <View accessible={true} accessibilityLabel={'AddWaitingListScreenDateView'} style={style.containerView2_2}>
                        <Text accessible={true} accessibilityLabel={'AddWaitingListScreenDateText'} preset={Text.preset.titleH3} style={style.text3}>{SGHelperType.formatDate(SGHelperType.convertNewDate(this.waitingListData.fBookDateTime), this._language)}</Text>
                    </View>
                    <View accessible={true} accessibilityLabel={'AddWaitingListScreenTimeView'} style={style.containerView2_2}>
                        <Text accessible={true} accessibilityLabel={'AddWaitingListScreenTimeText'} preset={Text.preset.titleH3} style={style.text3}>{SGHelperType.formatTime(SGHelperType.convertNewDate(this.waitingListData.fBookDateTime), this._language)}</Text>
                    </View>
                </View>
                ...
                {this.doneRenderProfileStore ?
                <ScrollView style={{ marginBottom: SGHelperWindow.getHeaderHeight()}} accessible={true} accessibilityLabel={'ViewWaitingListScreenScrollView'} contentContainerStyle={{ bacgroundColor: 'white' }}>
                    ...
                    {this.activeWaitingList === true && <Button accessible={true} accessibilityLabel={'ViewWaitingListScreenCalledButton'} disabled={this.waitingListData.fStatus === "called" ?? true} label={SGLocalize.translate('WaitingListAddView.CancelButtonText')} style={style.button} onPress={this._onCancelPress.bind(this)}></Button>}
                </ScrollView>
                :
                <ActivityIndicator preset={ActivityIndicator.preset.h1} style={{ flex: 1 }}></ActivityIndicator>
                }
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [SGHelperWindow.getWHPNoHeader().h + 8 * p, SGHelperWindow.getWHPNoHeader().h - Math.max(SGHelperWindow.getFooterHeight(), 2 * p) - SGHelperWindow.getHeaderHeight()] }) },], height: Math.max(SGHelperWindow.getFooterHeight(), 2 * p) + SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <BottomNavigationContainer accessible={true} accessibilityLabel={'RestoWaitingListDetailScreenBottomNav'} blackTheme navigator={this.props.navigation} style={style.containerView3}></BottomNavigationContainer>
                </Animated.View>
            </RootView>
        );
    }
