...
import { SGView as View, SGText as Text, SGScrollView as ScrollView, SGImage as Image, SGButton as Button, SGRootView as RootView, SGIcon as Icon, SGDialogBox, SGActivityIndicator as ActivityIndicator } from '../../../core/control';
import { SGHelperGlobalVar, SGHelperType, SGHelperNavigation, SGHelperWindow,SGHelperErrorHandling } from '../../../core/helper';
...
export class RestoOrderMenuDetailScreen extends SGBaseScreen {
    ...
    constructor(props, context, ...args) {
        ...
        this.active = this.props.route.params.active;
        ...
        this.totaldiscountitem = 0;
        this.discTotalPrice =0;
    }
        ...
        for (var i = 0; i < this.dataOrder[0].fOrderGroup.length; i++) {
            for (var j = 0; j < this.dataOrder[0].fOrderGroup[i].fOrderDetail.length; j++) {
                if (this.dataOrder[0].fOrderGroup[i].fOrderDetail[j].fStatus !== "cancelled") {
                    ...
                    this.discTotalPrice = this.discTotalPrice + this.dataOrder[0].fOrderGroup[i].fOrderDetail[j].fDiscountPrice;
                }
            }
        }
    }
    ...
    _countTaxAndServices(){
        ...
        if(this.dataSetting.fShowService == 'Y'){
            this.servicePrice = this.dataOrder[0].fServicePrice;
        }
        if(this.dataSetting.fShowTax == 'Y'){
            this.taxPrice = this.dataOrder[0].fTaxPrice;
        }
        this.finalPrice = this.totalPrice + this.taxPrice + this.servicePrice - this.dataOrder[0].fDiscountPrice;
        this.discTotalPrice = this.discTotalPrice + this.dataOrder[0].fDiscountPrice;
    }
    ...
    async _getAllOrder() {
        ...
        this.baseInitAPIParallel(SGHelperGlobalVar.getVar("ResponTimes"), (() => { this.checkAPIBatchStatusAllDone(); }).bind(this),(()=>{  this.dbID2 = SGDialogBox.showLoading(SGLocalize.translate('AlertMessage.Waiting'));}));
        ...
        if (data.length !== 0) {
            try {
                this.dataOrderRapih.push({
                    ...
                    'MenuName': data[0].fOrderGroup[0].fOrderDetail[0].fProductJSON["fProductName" + this._language.toUpperCase()],
                    'totalprice': (data[0].fOrderGroup[0].fOrderDetail[0].fProductJSON.fDiscountPrice) * data[0].fOrderGroup[0].fOrderDetail.length,

                    'arrCustomize': []
                })

                this.discountPerItem.push(data[0].fOrderGroup[0].fOrderDetail[0].fDiscountPrice)

                if (data[0].fOrderGroup[0].fOrderDetail.length > 1) {
                    var total = 0;
                    var totaldisc = 0;
                    for (let index = 0; index < data[0].fOrderGroup[0].fOrderDetail.length; index++) {
                        total += data[0].fOrderGroup[0].fOrderDetail[index].fProductJSON.fDiscountPrice;
                        totaldisc += data[0].fOrderGroup[0].fOrderDetail[index].fDiscountPrice;
                    }
                    this.dataOrderRapih[0].totalprice = total;
                    this.discountPerItem[0] = totaldisc;
                    this.totaldiscountitem += totaldisc;
                }
                ...
                if (data[0].fOrderGroup[0].fOrderDetail[0].fOrderCustomize[0] !== undefined) {
                    for (let index = 0; index < data[0].fOrderGroup[0].fOrderDetail[0].fOrderCustomize.length; index++) {
                        this.dataOrderRapih[0].arrCustomize.push({
                            'nameCustomize': data[0].fOrderGroup[0].fOrderDetail[0].fOrderCustomize[index].fCustomizationJSON["fCustomizeName" + this._language.toUpperCase()],
                            'qty': data[0].fOrderGroup[0].fOrderDetail.length,
            ...
            for (let index = 1; index < data[0].fOrderGroup.length; index++) {
                if (this.dataOrderRapih[i].fID !== data[0].fOrderGroup[index].fOrderDetail[0].fProductJSON.fID) {
                    this.dataOrderRapih.push({
                        ...
                        'MenuName': data[0].fOrderGroup[index].fOrderDetail[0].fProductJSON["fProductName" + this._language.toUpperCase()],
                    ...
                    this.discountPerItem.push(data[0].fOrderGroup[index].fOrderDetail[0].fDiscountPrice)
                    ...
                    if (data[0].fOrderGroup[index].fOrderDetail.length > 1) {
                        var total = 0;
                        var totaldisc = 0;
                        for (let j = 0; j < this.dataOrderRapih[i].qty; j++) {
                            total += data[0].fOrderGroup[index].fOrderDetail[j].fProductJSON.fDiscountPrice
                            totaldisc += data[0].fOrderGroup[index].fOrderDetail[j].fDiscountPrice
                        }
                        this.dataOrderRapih[i].totalprice = total;
                        this.discountPerItem[i] = totaldisc;
                        this.totaldiscountitem += totaldisc;
                    }
                } else {
                    ...
                    if (data[0].fOrderGroup[index].fOrderDetail.length > 1) {
                        var total = 0
                        var totaldisc = 0
                        for (let j = 0; j < data[0].fOrderGroup[index].fOrderDetail.length; j++) {
                            total += data[0].fOrderGroup[index].fOrderDetail[j].fProductJSON.fDiscountPrice
                            totaldisc += data[0].fOrderGroup[index].fOrderDetail[j].fDiscountPrice
                        }
                        this.dataOrderRapih[i].totalprice += total
                        ...
                        this.discountPerItem[i] += data[0].fOrderGroup[index].fOrderDetail[0].fDiscountPrice
                    }
                }
                ...
                if (data[0].fOrderGroup[index].fOrderDetail[0].fOrderCustomize[0] !== undefined) {
                    for (let j = 0; j < data[0].fOrderGroup[index].fOrderDetail[0].fOrderCustomize.length; j++) {
                        this.dataOrderRapih[i].arrCustomize.push({
                            'nameCustomize': data[0].fOrderGroup[index].fOrderDetail[0].fOrderCustomize[j].fCustomizationJSON["fCustomizeName" + this._language.toUpperCase()],
                            'qty': data[0].fOrderGroup[index].fOrderDetail.length,
        ...
        for (var i = 0; i < data.fOrderGroup.length; i++) {
            for (var j = 0; j < data.fOrderGroup[i].fOrderDetail.length; j++) {
                if (data.fOrderGroup[i].fOrderDetail[j].fStatus !== "deliver" && data.fOrderGroup[i].fOrderDetail[j].fStatus !== "cancelled" && data.fOrderGroup[i].fOrderDetail[j].fStatus !== "completed") {
                    { SGDialogBox.showFail(null, SGLocalize.translate("AlertMessage.Fail"), SGLocalize.translate('requestBillScreen.alertFailTextMenuInProcessing'), SGLocalize.translate("AlertMessage.Cancel"), () => { }), true }
    ...
    async _checkDeliverData(data){
        try {
            await tbVOrderMenuAPI.updateCompleteOrderVisitor(data)
            if (this.dataSetting.fOrderMenuDoneActionAlert === 1) {
                { SGDialogBox.showSuccess(null, SGLocalize.translate("AlertMessage.Success"), SGLocalize.translate('requestBillScreen.alertSuccessText'), SGLocalize.translate("AlertMessage.OK"), () => { SGHelperNavigation.navigatePop(this.props.navigation); this.props.route.params.callback() }, true) }
            } else {
                { SGDialogBox.showSuccess(null, SGLocalize.translate("AlertMessage.Success"), SGLocalize.translate('requestBillScreen.alertSuccessText2'), SGLocalize.translate("AlertMessage.OK"), () => { SGHelperNavigation.navigatePop(this.props.navigation); this.props.route.params.callback() }, true) }
            }
        } catch (error) {
            SGHelperErrorHandling.Handling(error,this._checkDeliverData.bind(this))
            // SGDialogBox.showSuccess(null, SGLocalize.translate("AlertMessage.Fail"), SGLocalize.translate('requestBillScreen.alertFailText'), SGLocalize.translate("AlertMessage.OK"), () => { }, true)
        }
    }
    ...
    async _onCompleteOrderPress() {
        ...
        if (this.checkDeliverData === false) {
            SGDialogBox.showConfirmation(null,  SGLocalize.translate("AlertMessage.Confirmation"), SGLocalize.translate("requestBillScreen.alertConfirmation"), SGLocalize.translate("AlertMessage.Cancel"), () => { },
                ...
                async () => {
                    await this._checkDeliverData(data)
                })
        }
    ...
    render() {
        ...
        console.log(this.dataOrderRapih);
        return (
            <RootView dummyStatusBar accessible={true} accessibilityLabel={'RequestBillScreenRootView'} style={style.v1}>
                                            ...
                                            return (
                                                <View>
                                                    ...
                                                    </View>
                                                    {
                                                     ...
                                                     <View accessible={true} accessibilityLabel={'HistoryDetailScreenBillTextView'} style={style.vbill}>
                                                        <View style={style.vbill1}></View>
                                                        <View accessible={true} accessibilityLabel={'HistoryDetailScreenBillProductNameView'} style={style.vbill2}>
                                                            <Text accessible={true} accessibilityLabel={'HistoryDetailScreenProductName'} preset={Text.preset.h7} style={style.textBillColorDisc}>Diskon</Text>
                                                        </View>
                                                        <View accessible={true} accessibilityLabel={'HistoryDetailScreenBillOrderPriceView'} style={style.vbill3}>
                                                            <Text accessible={true} accessibilityLabel={'HistoryDetailScreenBillOrderPriceText'} preset={Text.preset.h7} style={style.textBillColorDisc}>{this.dataSetting.fCurrency} {SGHelperType.addThousandSeparator((this.discountPerItem[i]).toString())}</Text>
                                                        </View>
                                                    </View>
                                                </View>
                                            )
                ...
                <View style={style.footerPrice}>
                    {(this.dataSetting.fShowTax === 'Y' || this.dataSetting.fShowService === 'Y') &&
                        <View style={style.billView}>
                            ...
                            {this.dataSetting.fShowTax === 'Y' &&
                             ...
                             {/* Discount  */}
                             {this.discTotalPrice !== undefined &&
                                <View accessible={true} accessibilityLabel={'CashierDetailScreenBillBottomView'} style={style.vbill} key={SGHelperType.getGUID()}>
                                    <View accessible={true} accessibilityLabel={'CashierDetailScreenBillTotalView'} style={style.vbill2}>
                                        <Text accessible={true} accessibilityLabel={'CashierDetailScreenBillTotalText'} preset={Text.preset.h6} style={style.textBillColorDisc}>{SGLocalize.translate('orderMenuListScreen.AdditionalDiscount')}</Text>
                                    </View>
                                    <View accessible={true} accessibilityLabel={'CashierDetailScreenBillTotalPriceView'} style={style.vbill3}>
                                        <Text accessible={true} accessibilityLabel={'CashierDetailScreenBillTotalPriceText'} preset={Text.preset.h6} style={style.textBillColorDisc}>{this.dataSetting.fCurrency}  {SGHelperType.addThousandSeparator((this.discTotalPrice-this.totaldiscountitem).toFixed(0))}</Text>
                                    </View>
                                </View>
                            }
                    ...
                    {this.discTotalPrice !== undefined &&
                        <View accessible={true} accessibilityLabel={'CashierDetailScreenBillBottomView'} style={style.vbill} key={SGHelperType.getGUID()}>
                            <View accessible={true} accessibilityLabel={'CashierDetailScreenBillTotalView'} style={style.vbill2}>
                                <Text accessible={true} accessibilityLabel={'CashierDetailScreenBillTotalText'} preset={Text.preset.h6} style={style.textBillColorDisc}>{SGLocalize.translate('orderMenuListScreen.TotalDiscount')}</Text>
                            </View>
                            <View accessible={true} accessibilityLabel={'CashierDetailScreenBillTotalPriceView'} style={style.vbill3}>
                                <Text accessible={true} accessibilityLabel={'CashierDetailScreenBillTotalPriceText'} preset={Text.preset.h6} style={style.textBillColorDisc}>{this.dataSetting.fCurrency}  {SGHelperType.addThousandSeparator((this.discTotalPrice).toFixed(0))}</Text>
                            </View>
                        </View>
                    }
