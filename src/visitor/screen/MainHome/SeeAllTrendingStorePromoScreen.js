...
export class SeeAllTrendingStorePromoScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getUserTrendingStorePromoList', (async (v1,v2,v3) => { return VtrendingAPI.getUserTrendingStorePromoList(v1,v2,v3) }).bind(this,this._filterData, this._sortData,this.paging), ((v) => {
                this.data = v; 
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getUserTrendingStorePromoList', (async (v1,v2,v3) => { return VtrendingAPI.getUserTrendingStorePromoList(v1,v2,v3) }).bind(this,this._filterData, this._sortData,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        if(!this.state.loading && !this.state.stopPulling){
            ...
            this.paging = this.getPagingData()
            this.baseRunSingleAPIWithRedoOption('getUserTrendingStorePromoList', (async (v1,v2,v3) => { return VtrendingAPI.getUserTrendingStorePromoList(v1,v2,v3) }).bind(this,this._filterData, this._sortData,this.paging), ((v) => {
                var resData = v// await VtrendingAPI.getUserTrendingStorePromoList(this._filterData, this._sortData,this.paging);
                if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        this._sortData = sortDAO.getStorePromoTrendingSortData(this.Language.toUpperCase());
        ...
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SeeAllTrendingStorePromoScreenRootView'} style={style.mainContainer}>
                ...
                {this.alreadyMount ?
                    ...
                    <NoSortSearchBar accessible={true} accessibilityLabel={'SeeAllTrendingStorePromoScreenNoSortSearchBar'} screen='SearchResultStorePromo' searchKeyword={this.searchKeyword} language={this.Language} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { this._setSort(v) } }) }} navigator={this.props.navigation} imageSetting={this.imageSetting} style={this.style.containerView1}></NoSortSearchBar>
                    {/* <NoSortSearchBar accessible={true} accessibilityLabel={'SeeAllTrendingStorePromoScreenNoSortSearchBar'} screen='SeeAllTrendingStorePromo' searchKeyword={this.searchKeyword} language={this.Language} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { this._setSort(v) } }) }} navigator={this.props.navigation} imageSetting={this.imageSetting} style={this.style.containerView1}></NoSortSearchBar> */}
                </Animated.View>
