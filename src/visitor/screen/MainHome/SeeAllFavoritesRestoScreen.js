...
export class SeeAllFavoritesRestoScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getUserFavRestoList', (async (v1,v2,v3) => { return tbVUserFavoriteAPI.getUserFavRestoList(v1,v2,v3) }).bind(this,this._filterData, this._sortData,this.paging), ((v) => {
                this.data = v; 
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getUserFavRestoList', (async (v1,v2,v3) => { return tbVUserFavoriteAPI.getUserFavRestoList(v1,v2,v3) }).bind(this,this._filterData, this._sortData,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        if(!this.state.loading && !this.state.stopPulling){
            ...
            this.paging = this.getPagingData()
            this.baseRunSingleAPIWithRedoOption('getUserFavRestoList', (async (v1,v2,v3) => { return tbVUserFavoriteAPI.getUserFavRestoList(v1,v2,v3) }).bind(this,this._filterData, this._sortData,this.paging), ((v) => {
                var resData = v// await tbVUserFavoriteAPI.getUserFavRestoList(this._filterData, this._sortData,this.paging);
                if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        this._sortData = sortDAO.getRestoFavoriteSortData(this.Language.toUpperCase());
        ...
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SeeAllFavoritesRestoScreenRootView'} style={style.mainContainer}>
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar accessible={true} accessibilityLabel={'SeeAllFavoritesRestoScreenSearchBar'} screen='SearchResultFavoriteResto' searchKeyword={this.searchKeyword} language={this.Language} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} imageSetting={this.imageSetting} style={this.style.containerView1}></FullSearchBar>
                </Animated.View>
