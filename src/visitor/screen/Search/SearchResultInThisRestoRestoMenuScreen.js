...
export class SearchResultInThisRestoRestoMenuScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getSearchInRestoRestoProduct', (async (v1,v2,v3,v4,v5,v6) => { return VSearchInThisRestoAPI.getSearchInRestoRestoProduct(v1,v2,v3,v4,v5,v6); }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData, this.restoKey,this.paging), ((v) => {
                this.data = v;
                if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
                else this.setState({stopPulling:false})
            }).bind(this),  null);
    
            // VSearchInThisRestoAPI.getSearchInRestoRestoProduct(this.Language, this.searchKeyword, this._filterData, this._sortData, this.restoKey,this.paging).then((v) => {
            //     this.data = v;
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getSearchInRestoRestoProduct', (async (v1, v2, v3, v4, v5,v6) => { return VSearchInThisRestoAPI.getSearchInRestoRestoProduct(v1, v2, v3, v4, v5,v6) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData, this.restoKey,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
            if(!this.state.loading && !this.state.stopPulling){
                ...
                this.baseRunSingleAPIWithRedoOption('getSearchInRestoRestoProduct', (async (v1, v2, v3,v4,v5,v6) => { return VSearchInThisRestoAPI.getSearchInRestoRestoProduct(v1, v2, v3,v4,v5,v6) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData, this.restoKey,this.paging), ((v) => {           
                    var resData = v// await VSearchInThisRestoAPI.getSearchInRestoRestoProduct(this.Language, this.searchKeyword, this._filterData, this._sortData, this.restoKey,this.paging);
                    if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        if(this.props.route.params.categoryKey && this.props.route.params.categoryKey !== 'trending' && this.props.route.params.categoryKey !== 'bestseller'){
            this._filterData = filterDAO.getInThisRestoRestoMenuFilterData(this.props.route.params.categoryKey);
        }
        else{
            this._filterData = filterDAO.getInThisRestoRestoMenuData();
        }
        this._sortData = sortDAO.getRestoMenuSearchInRestoSortData(this.Language.toUpperCase());
    ...
    render() {
        ...
        console.log('here')
        console.log('SearchResultInThisRestoRestoMenuScreen')
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultRestoProductScreenRootView'} style={style.mainContainer}>
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar categoryKey={this.props.route.params.categoryKey} accessible={true} accessibilityLabel={'SearchResultRestoMenuScreenFullSearchBar'} screen={'SearchResultInThisRestoRestoMenu'} restoKey={this.restoKey} searchKeyword={this.searchKeyword} language={this.Language} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} style={this.style.containerView1}></FullSearchBar>
                </Animated.View>
