...
import { VSearchResultRestoAPI } from '../../api/VSearchResultRestoAPI';
...
export class SearchResultRestoScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getAllSearchResultResto', (async (v1,v2,v3,v4,v5) => { return VSearchResultRestoAPI.getAllSearchResultResto(v1,v2,v3,v4,v5); }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
                this.data = v;
                if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
                else this.setState({stopPulling:false})
            }).bind(this),  null);

            // VSearchResultRestoAPI.getAllSearchResultResto(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging).then((v) => {
            //     this.data = v;
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getAllSearchResultResto', (async (v1, v2, v3, v4, v5) => { return VSearchResultRestoAPI.getAllSearchResultResto(v1, v2, v3, v4, v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        // try {
            ...
            if(!this.state.loading && !this.state.stopPulling){
                ...
                this.baseRunSingleAPIWithRedoOption('getAllSearchResultResto', (async (v1, v2, v3,v4,v5) => { return VSearchResultRestoAPI.getAllSearchResultResto(v1, v2, v3,v4,v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {           
                    var resData = v// await VSearchResultRestoAPI.getAllSearchResultResto(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging);
                    if(resData.length!==0){
    ...
    render() {
        ...
        console.log('SearchResultRestoScreen')
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultRestoScreenRootView'} style={style.mainContainer}>
                <RibbonHeader imageSetting={this.imageSetting} title={SGLocalize.translate("SearchResultRestoScreen.screenTitle")} ribbonWidth={0.65}></RibbonHeader>
                {this.alreadyMount ?
                    this.data.length !== 0 ?
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar accessible={true} forceUpdate={this._forceUpdate.bind(this)} screen='SearchResultResto' accessibilityLabel={'SearchResultRestoScreenFullSearchBar'} searchKeyword={this.searchKeyword} language={this.Language} style={this.style.containerView1} navigator={this.props.navigation} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }} filterData={this._filterData} buildingMatrix={this._buildingMatrix} onApplyFilter={(v) => { this._setFilter(v) }} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { this._setSort(v) } }) }}></FullSearchBar>
                </Animated.View>
