...
import { VSearchResultStoreAPI } from '../../api/VSearchResultStoreAPI';
...
export class SearchResultStoreScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getAllSearchResultStore', (async (v1,v2,v3,v4,v5) => { return VSearchResultStoreAPI.getAllSearchResultStore(v1,v2,v3,v4,v5); }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
                this.data = v;
            ...
            // VSearchResultStoreAPI.getAllSearchResultStore(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging).then((v) => {
            //     this.data = v;
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getAllSearchResultStore', (async (v1, v2, v3, v4, v5) => { return VSearchResultStoreAPI.getAllSearchResultStore(v1, v2, v3, v4, v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        // try {
            ...
            if(!this.state.loading && !this.state.stopPulling){
                ...
                this.baseRunSingleAPIWithRedoOption('getAllSearchResultStore', (async (v1, v2, v3,v4,v5) => {  return VSearchResultStoreAPI.getAllSearchResultStore(v1, v2, v3,v4,v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {           
                    var resData = v// await VSearchResultStoreAPI.getAllSearchResultStore(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging);
                    if(resData.length!==0){
        ...
        // } catch (error) {
        //     SGHelperErrorHandling.Handling(error,this._onLoad.bind)
        // }
        // finally{
        //     this.setState({loading:false})
        // }
    ...
    constructor(props, context, ...args) {
        ...
        this._sortData = sortDAO.getStoreSortData(this.Language.toUpperCase());
    ...
    render() {
        ...
        console.log('search result store screen')
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultStoreScreenRootView'} style={style.mainContainer}>
                <RibbonHeader imageSetting={this.imageSetting} title={SGLocalize.translate("SearchResultStoreScreen.screenTitle")}></RibbonHeader>
                {this.alreadyMount ?
                    this.data.length !== 0 ?
                        <FlatList refreshing={this.state.refreshFlatList} onRefresh={this._onRefresh.bind(this)} accessible={true} accessibilityLabel={'SearchResultStoreScreenList'} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingVertical: p, justifyContent: 'flex-start', alignItems: 'center' }} data={this.data} renderItem={({ item }) => {
                            return (
                                <DefaultStoreCard onNotification={((item, s)=>{ console.log(item);  item.fUserNotificationThis = s; this.forceUpdate();}).bind(this,item)} onFavorite={((item, s)=>{console.log(item);  item.fUserFavoriteThis = s; this.forceUpdate();}).bind(this,item)} onLike={((item, s,c)=>{item.fUserLikedThis = s; item.fLikeCountStore+=c; this.forceUpdate();}).bind(this,item)} accessible={true} accessibilityLabel={'SearchResultStoreScreenDefStoreCard'} likePackage={this._getLikeResourceStore(item)} navigator={this.props.navigation} language={this.Language} imageSetting={this.imageSetting} likeText={SGLocalize.translate("globalText.likeCountText")} locationText={SGLocalize.translate("SearchResultStoreScreen.locationText")} lastVisitedText={SGLocalize.translate("SearchResultStoreScreen.lastVisitedText")} key={item.key} contentKey={item.key} storeName={item['fStoreName' + (this.Language).toUpperCase()]} storeCategory={item.storeCategory} storeLocation={item['fContentStore' + this.Language.toUpperCase()].fLocation} storeFloor={item.floor} placeName={item['fBuildingName' + (this.Language).toUpperCase()]} image={item['fContentBuilding' + (this.Language).toUpperCase()].fImageJSON[0][this.imageSetting].uri} location={item['fContentImage']} lastVisited={item.lastVisited} contentLikeCount={item.fLikeCountStore} footerLikeCount={item.fLikeCountBuilding} contentImage={item['fContentStore' + (this.Language).toUpperCase()].fStoreImageJSON[0][this.imageSetting].uri} city={item.fCity} like={item.fUserLikedThis} notification={item.fUserNotificationThis} favorite={item.fUserFavoriteThis} style={style.containerView1}></DefaultStoreCard>
                            );
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar accessible={true} accessibilityLabel={'SearchResultStoreScreenFullSearchBar'} screen='SearchResultStore' searchKeyword={this.searchKeyword} language={this.Language} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} style={this.style.containerView1}></FullSearchBar>
                </Animated.View>
