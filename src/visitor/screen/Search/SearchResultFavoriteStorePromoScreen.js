...
import { VSearchResultFavoriteStorePromoAPI } from '../../api/VSearchResultFavoriteStorePromoAPI';
import { VFilterOptionsAPI } from '../../api/VFilterOptionsAPI';
import { EmptyDetailView } from '../../container_V2/EmptyDetailView';

export class SearchResultFavoriteStorePromoScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getAllSearchResultFavoriteStorePromo', (async (v1,v2,v3,v4,v5) => { return VSearchResultFavoriteStorePromoAPI.getAllSearchResultFavoriteStorePromo(v1,v2,v3,v4,v5); }).bind(this,this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
                this.data = v;
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getAllSearchResultFavoriteStorePromo', (async (v1, v2, v3, v4, v5) => { return VSearchResultFavoriteStorePromoAPI.getAllSearchResultFavoriteStorePromo(v1, v2, v3, v4, v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        if(!this.state.loading && !this.state.stopPulling){
            ...
            this.baseRunSingleAPIWithRedoOption('getAllSearchResultFavoriteStorePromo', (async (v1, v2, v3,v4,v5) => {  return VSearchResultFavoriteStorePromoAPI.getAllSearchResultFavoriteStorePromo(v1, v2, v3,v4,v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {           
                var resData =  v//await VSearchResultFavoriteStorePromoAPI.getAllSearchResultFavoriteStorePromo(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging);
                if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        this._sortData = sortDAO.getStoreFavoritePromoSearchSortData(this.Language.toUpperCase());
    ...
    _getCommentResourceStorePromo(data) {
        ...
        return (
            {
                fUserImage: this.currentUserData.fProfileImageJSON, fContentType: 'StorePromo', fContentKey: data.eventKey,
        ...
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultFavoriteStorePromoScreenRootView'} style={style.mainContainer}>
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar accessible={true} accessibilityLabel={'SearchResultFavoriteStorePromoScreenFullSearchBar'} screen={'SearchResultFavoriteStorePromo'} searchKeyword={this.searchKeyword} language={this.Language} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} style={this.style.containerView1}></FullSearchBar>
                </Animated.View>
