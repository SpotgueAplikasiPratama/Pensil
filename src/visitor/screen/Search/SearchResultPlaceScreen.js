...
import { VSearchResultPlaceAPI } from '../../api/VSearchResultPlaceAPI';
...
export class SearchResultPlaceScreen extends SGBaseScreen {

    getPagingData(){
        var itemPerPage = SGHelperType.getPaging()
        return {paging:true,offset:this.pagingCounter, totalPerPage:SGHelperType.getPaging()}
    }
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getAllSearchResultPlace', (async (v1,v2,v3,v4,v5) => { return VSearchResultPlaceAPI.getAllSearchResultPlace(v1,v2,v3,v4,v5); }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
                this.data = v;
                if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
                else this.setState({stopPulling:false})
            }).bind(this),  null);
    
            // VSearchResultPlaceAPI.getAllSearchResultPlace(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging).then((v) => {
            //     this.data = v;
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getAllSearchResultPlaceEvent', (async (v1, v2, v3, v4, v5) => { return VSearchResultPlaceAPI.getAllSearchResultPlace(v1, v2, v3, v4, v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        // try {
            ...
            if(!this.state.loading && !this.state.stopPulling){
                ...
                this.baseRunSingleAPIWithRedoOption('getAllSearchResultPlace', (async (v1, v2, v3,v4,v5) => { return VSearchResultPlaceAPI.getAllSearchResultPlace(v1, v2, v3,v4,v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {           
                    var resData = v//  await VSearchResultPlaceAPI.getAllSearchResultPlace(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging);
                    if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        this._sortData = sortDAO.getPlaceSearchSortData(this.Language.toUpperCase());
    ...
    render() {
        ...
        console.log('render')
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultPlaceScreenRootView'} style={style.mainContainer}>
                ...
                <RibbonHeader imageSetting={this.imageSetting} title={SGLocalize.translate("SearchResultPlaceScreen.screenTitle")}></RibbonHeader>
                {this.alreadyMount ?
                    this.data.length !== 0 ?
                        (<FlatList refreshing={this.state.refreshFlatList} onRefresh={this._onRefresh.bind(this)} accessible={true} accessibilityLabel={'SearchResultPlaceScreenList'} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingVertical: p, justifyContent: 'flex-start', alignItems: 'center' }} data={this.data} renderItem={({ item }) => {
                            return (
                                <DefaultPlaceCard onNotification={((item, s)=>{ console.log(item);  item.fUserNotificationThis = s; this.forceUpdate();}).bind(this,item)} onFavorite={((item, s)=>{console.log(item);  item.fUserFavoriteThis = s; this.forceUpdate();}).bind(this,item)} onLike={((item, s,c)=>{item.fUserLikedThis = s; item.fLikeCount+=c; this.forceUpdate();}).bind(this,item)} accessible={true} accessibilityLabel={'SearchResultPlaceScreenDefPlaceCard'} likePackage={this._getLikeResourcePlace(item)} navigator={this.props.navigation} imageSetting={this.imageSetting} likeText={SGLocalize.translate("SeeAllFavoritesPlaceScreen.likeText")} lastVisitedText={SGLocalize.translate("SeeAllFavoritesPlaceScreen.lastVisitedText")} likeIconLabel={SGLocalize.translate("SeeAllFavoritesPlaceScreen.likeIconLabel")}
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar accessible={true} accessibilityLabel={'SearchResultPlaceScreenFullSearchBar'} screen='SearchResultPlace'  searchKeyword={this.searchKeyword} language={this.Language} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }}  onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} style={this.style.containerView1}></FullSearchBar>
                </Animated.View>
