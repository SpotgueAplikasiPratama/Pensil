...
export class SearchResultInThisPlaceStoreScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getSearchInBuildingPlaceStore', (async (v1,v2,v3,v4,v5,v6) => { return VSearchInThisPlaceAPI.getSearchInBuildingPlaceStore(v1,v2,v3,v4,v5,v6); }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData, this.buildingKey,this.paging), ((v) => {
                this.data = v;
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getSearchInBuildingPlaceStore', (async (v1, v2, v3, v4, v5,v6) => { return VSearchInThisPlaceAPI.getSearchInBuildingPlaceStore(v1, v2, v3, v4, v5,v6) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData, this.buildingKey,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    constructor(props, context, ...args) {
        ...
        this._sortData = sortDAO.getStoreSearchSortData(this.Language.toUpperCase());
        ...
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultStoreScreenRootView'} style={style.mainContainer}>
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar accessible={true} accessibilityLabel={'SearchResultStoreScreenFullSearchBar'} screen={'SearchResultInThisPlaceStore'} buildingKey={this.buildingKey} language={this.Language} searchKeyword={this.searchKeyword} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} style={this.style.containerView1}></FullSearchBar></Animated.View>
