...
/**.
 * Version 1.2.0
 * Change Log By Melvin - > Add Paging
 * 29 March 2021
 ...
 * - add ErrorHandling
 * Version 1.1.0
    Change Log - > Fixing Card Resto Home
...
import { VSearchResultStorePromoAPI } from '../../api/VSearchResultStorePromoAPI';
import { VFilterOptionsAPI } from '../../api/VFilterOptionsAPI';
import { EmptyDetailView } from '../../container_V2/EmptyDetailView';

export class SearchResultStorePromoScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getAllSearchResultStorePromo', (async (v1,v2,v3,v4,v5) => { return VSearchResultStorePromoAPI.getAllSearchResultStorePromo(v1,v2,v3,v4,v5); }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
                this.data = v;
                if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
                else this.setState({stopPulling:false})
            }).bind(this),  null);

            // VSearchResultStorePromoAPI.getAllSearchResultStorePromo(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging).then((v) => {
            //     this.data = v;
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getAllSearchResultStorePromo', (async (v1, v2, v3, v4, v5) => { return VSearchResultStorePromoAPI.getAllSearchResultStorePromo(v1, v2, v3, v4, v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        // try {
            if(!this.state.loading && !this.state.stopPulling){
                ...
                this.baseRunSingleAPIWithRedoOption('getAllSearchResultStorePromo', (async (v1, v2, v3,v4,v5) => {  return VSearchResultStorePromoAPI.getAllSearchResultStorePromo(v1, v2, v3,v4,v5) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging), ((v) => {           
                    var resData = v// await VSearchResultStorePromoAPI.getAllSearchResultStorePromo(this.Language, this.searchKeyword, this._filterData, this._sortData,this.paging);
                    if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        this.data = [];
        this._filterData = filterDAO.getStorePromoFilterDataSearch(this.Language);
        this._sortData = sortDAO.getStorePromoSearchSortData(this.Language.toUpperCase());
    ...
    render() {
        ...
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultStorePromoScreenRootView'} style={style.mainContainer}>
                <RibbonHeader imageSetting={this.imageSetting} title={SGLocalize.translate("SearchResultStorePromoScreen.screenTitle")}></RibbonHeader>
                {this.alreadyMount ?
                    this.data.length !== 0 ?
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar accessible={true} accessibilityLabel={'SearchResultStorePromoScreenFullSearchBar'} screen='SearchResultStorePromo' searchKeyword={this.searchKeyword} language={this.Language} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} style={this.style.containerView1}></FullSearchBar>
                </Animated.View>
