...
export class SearchResultInThisStoreStoreProductScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('getSearchInStoreStoreProduct', (async (v1,v2,v3,v4,v5,v6) => { return VSearchInThisStoreAPI.getSearchInStoreStoreProduct(v1,v2,v3,v4,v5,v6); }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData, this.storeKey,this.paging), ((v) => {
                this.data = v;
                if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
                else this.setState({stopPulling:false})
            }).bind(this),  null);

            // VSearchInThisStoreAPI.getSearchInStoreStoreProduct(this.Language, this.searchKeyword, this._filterData, this._sortData, this.storeKey,this.paging).then((v) => {
            //     this.data = v;
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getSearchInStoreStoreProduct', (async (v1, v2, v3, v4, v5,v6) => { return VSearchInThisStoreAPI.getSearchInStoreStoreProduct(v1, v2, v3, v4, v5,v6) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData, this.storeKey,this.paging), ((v) => {
            this.data = v
            this.pagingCounter = this.data.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        // try {
            if(!this.state.loading && !this.state.stopPulling){
                ...
                this.baseRunSingleAPIWithRedoOption('getSearchInStoreStoreProduct', (async (v1, v2, v3,v4,v5,v6) => { return VSearchInThisStoreAPI.getSearchInStoreStoreProduct(v1, v2, v3,v4,v5,v6) }).bind(this, this.Language, this.searchKeyword, this._filterData, this._sortData, this.storeKey,this.paging), ((v) => {           
                var resData = v// await VSearchInThisStoreAPI.getSearchInStoreStoreProduct(this.Language, this.searchKeyword, this._filterData, this._sortData, this.storeKey,this.paging);
                if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        if(this.props.route.params.categoryKey && this.props.route.params.categoryKey !== 'trending'){
            this._filterData = filterDAO.getInThisStoreStoreProductFilterData(this.props.route.params.categoryKey);
        }
        else{
            this._filterData = filterDAO.getInThisStoreStoreProductData();
        }
        this._sortData = sortDAO.getStoreProductSearchInStoreSortData();
    ...
    render() {
        ...
        console.log('search result in this store product screen')
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultStoreProductScreenRootView'} style={style.mainContainer}>
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <FullSearchBar categoryKey = {this.props.route.params.categoryKey} accessible={true} accessibilityLabel={'SearchResultRestoMenuScreenFullSearchBar'} screen={'SearchResultInThisStoreStoreProduct'} storeKey={this.storeKey} searchKeyword={this.searchKeyword} language={this.Language} sortData={this._sortData} onApplySort={(v) => { this._setSort(v) }} onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} style={this.style.containerView1}></FullSearchBar>
                </Animated.View>
