...
import { SGView as View, SGRootView as RootView, SGText as Text, SGImage as Image, SGTouchableOpacity as TouchableOpacity, SGViewPager as ViewPager, SGFlatList as FlatList, SGPopView, SGButton as Button, SGActivityIndicator as ActivityIndicator, SGScrollView as ScrollView } from '../../../core/control';
...
import { VSearchAllAPI } from '../../api/VSearchAllAPI';
        ...
        return (
            <View accessible={true} accessibilityLabel={'ContentHeaderMainView'} style={style.mainContainer}>
                ...
                <View accessible={true} style={{ alignSelf: 'flex-end', marginRight: p * 3 }}>
                    <TouchableOpacity onPress={() => SGHelperNavigation.navigatePush(this.props.navigator, this.props.screen, { searchKeyword: this.props.searchKeyword })}>
...
export class SearchAllScreen extends SGBaseScreen {
    ...
    createStyleSheet = (whp) => {
        ...
        return StyleSheet.create({
            ...
            placeSliderContainer: { width: w, height: w * 0.429, backgroundColor: 'transparent' },
    ...
    async _onRefreshAllItem() { 
        ...
        this.baseAddAPIParallel('getSearchAllBuildingSlider', (async (v1,v2,v3) => { return VSearchAllAPI.getSearchAllBuildingSlider(v1,v2,v3); }).bind(this,this.Language, this.searchKeyword, this._sortDataPlace), ((v) => {
            this.dataPlace = v;
        }).bind(this),  null);

        this.baseAddAPIParallel('getSearchAllStoreSlider', (async (v1,v2,v3) => { return VSearchAllAPI.getSearchAllStoreSlider(v1,v2,v3); }).bind(this,this.Language, this.searchKeyword, this._sortDataStore), ((v) => {
            this.dataStore = v;
        }).bind(this),  null);
        
        this.baseAddAPIParallel('getSearchAllRestoSlider', (async (v1,v2,v3) => { return VSearchAllAPI.getSearchAllRestoSlider(v1,v2,v3); }).bind(this,this.Language, this.searchKeyword, this._sortDataResto), ((v) => {
            this.dataResto = v;
        }).bind(this),  null);

        this.baseAddAPIParallel('getSearchAllBuildingEventSlider', (async (v1,v2,v3) => { return VSearchAllAPI.getSearchAllBuildingEventSlider(v1,v2,v3); }).bind(this,this.Language, this.searchKeyword, this._sortDataPlaceEvent), ((v) => {
            this.dataPlaceEvent = v;
        }).bind(this),  null);

        this.baseAddAPIParallel('getSearchAllStorePromoSlider', (async (v1,v2,v3) => { return VSearchAllAPI.getSearchAllStorePromoSlider(v1,v2,v3); }).bind(this,this.Language, this.searchKeyword, this._sortDataStorePromo), ((v) => {
            this.dataStorePromo = v;
        }).bind(this),  null);

        this.baseAddAPIParallel('getSearchAllRestoPromoSlider', (async (v1,v2,v3) => { return VSearchAllAPI.getSearchAllRestoPromoSlider(v1,v2,v3); }).bind(this, this.Language, this.searchKeyword, this._sortDataRestoPromo), ((v) => {
    ...
    constructor(props, context, ...args) {
        ...
        this._sortDataPlace = sortDAO.getPlaceSearchSortData(this.Language.toUpperCase());
    ...
    render() {
        ...
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchAllScreenRootView'} style={style.mainContainer}>
                ...
                {this.alreadyMount ?
                    this.dataPlace.length !== 0 || this.dataStore.length !== 0 || this.dataResto.length !== 0 || this.dataPlaceEvent.length || this.dataStorePromo.length !== 0 || this.dataRestoPromo.length !== 0 ?
                        (<ScrollView dummyFooterBar dummyBottomBar accessible={true} accessibilityLabel={'SearchAllScreenScrollView'} style={style.scrollViewContainer} showsVerticalScrollIndicator={false} contentContainerStyle={style.scrollViewContainerContent}>
                            {this.dataPlace.length !== 0 ?
                                (<View style={{ marginBottom: p * 1.3 }}>
                                    <ContentHeader accessible={true} accessibilityLabel={'SearchAllScreenContentHeaderPlace'} navigator={this.props.navigation} screen={'SearchResultPlace'} title={SGLocalize.translate("SearchAllScreen.placeTitle")} count={this.dataPlace.length} imageSetting={this.imageSetting} searchKeyword={this.searchKeyword} currentUser={this.currentUser} style={style.throwWHP}></ContentHeader>
                                    <ViewPager accessible={true} accessibilityLabel={'SearchAllPlaceSliderViewPager'} style={style.placeSliderContainer} initialPage={0} showPageIndicator={true} transitionStyle={'scroll'} pageIndicatorStyle={{ position: 'absolute', bottom: 0, marginVertical: 0, flexDirection: 'row', alignSelf: 'center' }}>
                                        {
                                            this.dataPlace.map((x) => {
                                                return (
                                                    <View style={{ justifyContent: 'flex-start' }}>
                                                        <DefaultPlaceCard onNotification={((item, s)=>{ console.log(item);  item.fUserNotificationThis = s; this.forceUpdate();}).bind(this,x)} onFavorite={((item, s)=>{console.log(item);  item.fUserFavoriteThis = s; this.forceUpdate();}).bind(this, x)} onLike={((item, s,c)=>{item.fUserLikedThis = s; item.fLikeCount+=c; this.forceUpdate();}).bind(this,x)} slider accessible={true} accessibilityLabel={'SearchAllScreenDefPlaceCard'} likePackage={this._getLikeResourcePlace(x)} navigator={this.props.navigation} imageSetting={this.imageSetting} likeText={SGLocalize.translate("SeeAllFavoritesPlaceScreen.likeText")} lastVisitedText={SGLocalize.translate("SeeAllFavoritesPlaceScreen.lastVisitedText")} likeIconLabel={SGLocalize.translate("SeeAllFavoritesPlaceScreen.likeIconLabel")} notificationIconLabel={SGLocalize.translate("SeeAllFavoritesPlaceScreen.notificationIconLabel")} favoriteIconLabel={SGLocalize.translate("SeeAllFavoritesPlaceScreen.favoriteIconLabel")} key={x.key} contentKey={x.key} placeName={x['fContent' + this.Language.toUpperCase()].fBuildingName} placeCategory={x.fBuildingType} location={x.fCity} lastVisited={x.lastVisited} contentLikeCount={x.fLikeCount} contentImage={x['fContent' + this.Language.toUpperCase()].fImageJSON[0][this.imageSetting].uri} shortDesc={x['fContent' + this.Language.toUpperCase()].fShortDescription} like={x.fUserLikedThis} notification={x.fUserNotificationThis} favorite={x.fUserFavoriteThis} style={style.containerView1} likeMallGetReward={async () => await this._SurpriseRewardLikeMall(x.key)}></DefaultPlaceCard>
                                                    </View>
                                                )
                                            })
                                        }
                            ...
                            {this.dataStore.length !== 0 ?
                                (<View style={{ marginBottom: p * 1.3 }}>
                                    <ContentHeader accessible={true} accessibilityLabel={'SearchAllScreenContentHeaderStore'} navigator={this.props.navigation} screen={'SearchResultStore'} title={SGLocalize.translate("SearchAllScreen.storeTitle")} count={this.dataStore.length} imageSetting={this.imageSetting} searchKeyword={this.searchKeyword} currentUser={this.currentUser} style={style.throwWHP}></ContentHeader>
                            ...
                            {this.dataResto.length !== 0 ?
                                (<View style={{ marginBottom: p * 1.3 }}>
                                    <ContentHeader accessible={true} accessibilityLabel={'SearchAllScreenContentHeaderResto'} navigator={this.props.navigation} screen={'SearchResultResto'} title={SGLocalize.translate("SearchAllScreen.restoTitle")} count={this.dataResto.length} imageSetting={this.imageSetting} searchKeyword={this.searchKeyword} currentUser={this.currentUser} style={style.throwWHP}></ContentHeader>
                            ...
                            {this.dataPlaceEvent.length !== 0 ?
                                (<View style={{ marginBottom: p * 1.3 }}>
                                    <ContentHeader accessible={true} accessibilityLabel={'SearchAllScreenContentHeaderPlaceEvent'} navigator={this.props.navigation} screen={'SearchResultPlaceEvent'} title={SGLocalize.translate("SearchAllScreen.placeEventTitle")} count={this.dataPlaceEvent.length} imageSetting={this.imageSetting} searchKeyword={this.searchKeyword} currentUser={this.currentUser} style={style.throwWHP}></ContentHeader>
                            ...
                            {this.dataStorePromo.length !== 0 ?
                                (<View style={{ marginBottom: p * 1.3 }}>
                                    <ContentHeader accessible={true} accessibilityLabel={'SearchAllScreenContentHeaderStorePromo'} navigator={this.props.navigation} screen={'SearchResultStorePromo'} title={SGLocalize.translate("SearchAllScreen.storePromoTitle")} count={this.dataStorePromo.length} imageSetting={this.imageSetting} searchKeyword={this.searchKeyword} currentUser={this.currentUser} style={style.throwWHP}></ContentHeader>
                            ...
                            {this.dataRestoPromo.length !== 0 ?
                                (<View style={{ marginBottom: p * 1.3 }}>
                                    <ContentHeader accessible={true} accessibilityLabel={'SearchAllScreenContentHeaderStorePromo'} navigator={this.props.navigation} screen={'SearchResultRestoPromo'} title={SGLocalize.translate("SearchAllScreen.restoPromoTitle")} count={this.dataRestoPromo.length} imageSetting={this.imageSetting} searchKeyword={this.searchKeyword} currentUser={this.currentUser} style={style.throwWHP}></ContentHeader>
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    <SimpleSearchBar accessible={true} accessibilityLabel={'SearchAllScreenSimpleSearchBar'} imageSetting={this.imageSetting} language={this.Language} searchKeyword={this.searchKeyword} navigator={this.props.navigation} placeholder='' style={this.style.containerView1}></SimpleSearchBar>
                </Animated.View>
