...
export class SearchResultGroupRestoScreen extends SGBaseScreen {
    ...
    render() {
        ...
        console.log('SearchResultGroupResto')
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SearchResultGroupStoreScreenRootView'} style={style.mainContainer}>
                <RibbonHeader imageSetting={this.imageSetting} title={SGLocalize.translate('HomeScreen.groupRestoTabTitle')}></RibbonHeader>
                {this.alreadyMount ?
                    this.data.length !== 0 ?
                        <FlatList refreshing={this.state.refreshFlatList} onRefresh={this._onRefresh.bind(this)} accessible={true} accessibilityLabel={'SearchResultGroupStoreScreenList'} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingVertical: p, justifyContent: 'flex-start', alignItems: 'center' }} data={this.data} renderItem={({ item }) => {
                            return (
                                <DefaultRestoCard onNotification={((item, s)=>{ console.log(item);  item.fUserNotificationThis = s; this.forceUpdate();}).bind(this,item)} onFavorite={((item, s)=>{console.log(item);  item.fUserFavoriteThis = s; this.forceUpdate();}).bind(this,item)} onLike={((item, s,c)=>{ console.log(item); item.fUserLikedThis = s; var fLikeCountStore=parseInt(item.fLikeCountStore) + c; item.fLikeCountStore= (fLikeCountStore) ;this.forceUpdate();}).bind(this,item)} accessible={true} accessibilityLabel={'SearchResultRestoScreenDefRestoCard'} likePackage={this._getLikeResourceResto(item)} navigator={this.props.navigation} language={this.Language} imageSetting={this.imageSetting} likeText={SGLocalize.translate("SeeAllFavoritesRestoScreen.likeText")} locationText={SGLocalize.translate("SearchResultRestoScreen.locationText")} lastVisitedText={SGLocalize.translate("SeeAllFavoritesRestoScreen.lastVisitedText")} key={item.key} contentKey={item.key} restoName={item['fContentStore' + (this.Language).toUpperCase()].fStoreName} restoCategory={item.storeCategory} restoCuisine={item.restoCuisine} isHalal={item.isHalal} isVegetarian={item.isVegetarian} restoLocation={item['fContentStore' + this.Language.toUpperCase()].fLocation} storeFloor={item.floorName} placeName={item['fContentBuilding' + (this.Language).toUpperCase()].fBuildingName} image={item['fContentBuilding' + (this.Language).toUpperCase()].fImageJSON[0][this.imageSetting].uri} location={item.fCity} lastVisited={item.fLastVisited} contentLikeCount={item.fLikeCountStore} footerLikeCount={item.fLikeCountBuilding} contentImage={item['fContentStore' + (this.Language).toUpperCase()].fStoreImageJSON[0][this.imageSetting].uri} city={item.fCity} like={item.fUserLikedThis} notification={item.fUserNotificationThis} favorite={item.fUserFavoriteThis} style={style.containerView1}></DefaultRestoCard>
                            );
