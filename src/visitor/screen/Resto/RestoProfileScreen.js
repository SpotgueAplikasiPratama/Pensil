 ...
 */
  ...
  import { SGView as View, SGViewPager as ViewPager, SGPopView, SGScrollView as ScrollView, SGRootView as RootView, SGText as Text, SGImage as Image, SGTouchableOpacity as TouchableOpacity, SGTouchableOpacity, SGIcon as Icon, SGMapStatic, SGWebView as WebView, SGActivityIndicator as ActivityIndicator,SGDialogBox as DialogBox,SGPanZoomView,SGDragView  } from '../../../core/control';
  ...
  export class RestoProfileScreen extends SGBaseScreen {
      createStyleSheet = (whp) => {
          ...
          return StyleSheet.create({
              ...
              sliderContainer: { width: w, height: w * 1.15 * 9 / 16, backgroundColor: '#FFFFFF', marginTop: p * 4,  },
              sliderImage: { width: w, height: w * 9 / 16, resizeMode: 'cover', alignSelf: 'center', },
      ...
      async _onRefreshAllItem() {
          ...
          this.baseRunSingleAPIWithRedoOption('getRestoHomeProfile', (async (v1) => { return VRestoProfileAPI.getRestoHomeProfile(v1) }).bind(this,this.restoKey), ((v) => {
              this.data = v 
      ...
      _getCommentResource(data) {
          ...
          return (
              {
                  fUserImage: this.currentUserData.fProfileImageJSON, fContentType: 'Resto', fContentKey: data.key,
    ...
    getValue(data,index){
      ...
      render() {
          ...
          var shareParams = { MallName:data['fBuildingName' + this.Language.toUpperCase()] , StorestoName: this.data['fRestoName' +this.Language.toUpperCase()]}
          console.log(data);

          return (
              <RootView dummyStatusBar accessible={true} accessibilityLabel={'RestoProfileScreenRootView'} style={style.mainContainer}>
                  {this.isAlreadyMount ?
                      this.data !== '' ?
                          (<View accessible={true} style={style.mainContainer}>
                              <View accessible={true} style={style.coverImageContainer}>
                                  <Image accessible={true} accessibilityLabel={'RestoProfileScreenBackgroundImage'} style={style.coverImage} source={{ uri: dataContent.fStoreBackgroundImageJSON[0][this.imageSetting].uri }}>
                                      <View accessible={true} style={style.coverOverlay}></View>
                                  </Image>
                              </View>
                              <View style={{ width: w, flex: 1, backgroundColor: 'transparent' }}>
                                  <ScrollView dummyFooterBar dummyBottomBar bounces={false} accessible={true} accessibilityLabel={'RestoProfileScreenScrollView'} style={style.scrollView} contentContainerStyle={style.scrollContainer} scrollEventThrottle={16} onScroll={(v) => { this.onScrollHandler(v); this.baseOnScrollHandler(v); }} onMomentumScrollBegin={this.baseOnMomentumScrollBeginHandler.bind(this)} onScrollEndDrag={this.baseOnScrollEndDragHandler.bind(this)} onMomentumScrollEnd={this.baseOnMomentumScrollEndHandler.bind(this)} showsVerticalScrollIndicator={false}>
                                      ...
                                      <View accessible={true} style={style.profileContainer}>
                                          <View accessible={true} style={style.profileDetailContainer}>
                                              <Text accessible={true} accessibilityLabel={'RestoProfileScreenRestoName'} preset={Text.preset.titleH1B} style={style.nameText}>{dataContent.fStoreName.toUpperCase()}</Text>
                                              <Text accessible={true} accessibilityLabel={'RestoProfileScreenLikeCount'} preset={Text.preset.titleH4B} style={style.likedText}>{data.fLikeCount} {SGLocalize.translate('globalText.likeCountText')}</Text>
                                              {data.linkWebsite !== "" &&
                                              <TouchableOpacity onPress={() => this.handleLink(data.linkWebsite)}>
                                                  <Text accessible={true} accessibilityLabel={'RestoProfileScreenWebText'} style={style.websiteText} preset={Text.preset.titleH4_5B}>{SGLocalize.translate('restoProfileScreen.websiteText', { name: dataContent.fStoreName })}</Text>
                                              </TouchableOpacity>
                                              }
                                              <View accessible={true} accessibilityLabel={'StoreProfileScreenIconButtonView'} style={style.iconButttonContainer}>
                                                  <CardIconButtonFavorite onIconPressed={((item, s)=>{console.log(item);  item.fUserFavoriteThis = s; this.forceUpdate();}).bind(this, this.data)} accessible={true} accessibilityLabel={'RestoProfileScreenFavoriteIcon'} textColor='white' navigator={this.props.navigation} contentType='Resto' contentKey={this.restoKey} active={data.fUserFavoriteThis} type={'favorite'} ></CardIconButtonFavorite>
                                                  <CardIconButtonLike onIconPressed={((item, s,c)=>{console.log(item);  item.fUserLikedThis = s; item.fLikeCount+=c; this.forceUpdate();}).bind(this, this.data)} accessible={true} accessibilityLabel={'RestoProfileScreenLikeIcon'} likePackage={this._getLikeResource(this.data)} textColor='white' navigator={this.props.navigation} contentType='Resto' contentKey={this.restoKey} active={data.fUserLikedThis} type={'like'} ></CardIconButtonLike>
                                                  <CardIconButtonComment accessible={true} accessibilityLabel={'RestoProfileScreenCommentIcon'} commentPackage={this._getCommentResource(this.data)} textColor='white' navigator={this.props.navigation} contentType='Resto' contentKey={this.restoKey} canComment={data.fCanComment} type={'comment'} ></CardIconButtonComment>
                                                  <CardIconButtonShare shareParams={shareParams} img={dataContent.fStoreImageJSON[0].thumbnailLow.uri} accessible={true} accessibilityLabel={'RestoProfileScreenShareIcon'} textColor='white' navigator={this.props.navigation} contentType='Resto' contentKey={this.restoKey} shareMessage={dataContent.fShareMessage} targetKey={this.restoKey} type={'share'} ></CardIconButtonShare>
                                                  <CardIconButtonNotification onIconPressed={((item, s)=>{ console.log(item);  item.fUserNotificationThis = s; this.forceUpdate();}).bind(this, this.data)}  accessible={true} accessibilityLabel={'RestoProfileScreenNotificationIcon'} textColor='white' navigator={this.props.navigation} contentType='Resto' contentKey={this.restoKey} active={data.fUserNotificationThis} type={'notification'} ></CardIconButtonNotification>   
                                              </View>
                                          </View>
                                          ...
                                          <View accessible={true} style={style.profileInfoContainer}>
                                                          ...
                                                          return (
                                                              x.fActive === 'Y' ?
                                                                  (<View accessible={true} style={style.operationHoursContainer}>
                                                                      <Text accessible={true} accessibilityLabel={'RestoProfileScreenDayName'} style={style.sectionDescText} preset={Text.preset.titleH3}>{SGLocalize.translate('dayName.' + x.day)}</Text>
                                                                  ...
                                                                  (<View accessible={true} style={style.operationHoursContainer}>
                                                                      ...
                                                                      <View style={{width:w*0.3,justifyContent:'center'}}> 
                                                                        <Text accessible={true} accessibilityLabel={'RestoProfileScreenHourText'} style={style.operationalText} preset={Text.preset.titleH3}>{SGLocalize.translate('mallProfileScreen.closedText')}</Text>
                                                                      </View>
                                                                  </View>)
                                                          )
                                                      })}
                                            ...
                                            { ((data.whatsapp !== '' && data.whatsapp !== '-') || (data.phoneNumber !== '' && data.phoneNumber !== '-') 
                                              ...
                                              <View accessible={true} style={style.infoSectionContainer}>
                                                  <Text accessible={true} accessibilityLabel={'RestoProfileScreenPhoneNumbTitle'} style={style.infoSectionHeadingText} preset={Text.preset.titleH2B}>{SGLocalize.translate('storeProfileScreen.contacts')}</Text>
                                                  <View accessible={true} style={style.infoSectionDescContainer}>
                                                      {(data.phoneNumber !== '' && data.phoneNumber !== '-') &&
                                                        <Text accessible={true} accessibilityLabel={'RestoProfileScreenPhoneNumbText'} style={style.sectionDescText} preset={Text.preset.titleH3}>{data.phoneNumber}</Text>
                                                      }

                                                      {(data.email !== '' && data.email !== '-') && 
                                                      <Text accessible={true} accessibilityLabel={'RestoProfileScreenEmailText'} style={style.contentTextBlue} preset={Text.preset.titleH3}>{data.email}</Text>
                                                      }
                                              ...
                                              {/* Lokasi  */}
                                              <View accessible={true} style={style.infoSectionContainer}>
                                                  <Text accessible={true} accessibilityLabel={'RestoProfileScreenLocTitle'} style={style.infoSectionHeadingText} preset={Text.preset.titleH2B}>{SGLocalize.translate('restoProfileScreen.location')}</Text>
                                                  
                                                  {/* Tipe Gamber  */}
                                                  {this.noImageLocation !== true &&
                                                   data.fShowLayoutLocationType == 'type2' &&
                                                      <ViewPager accessible={true} accessibilityLabel={'LocationImageViewPager'} style={style.sliderContainer} initialPage={0} showPageIndicator={true} transitionStyle={'scroll'} pageIndicatorStyle={{ position: 'absolute', bottom: 0 * p, flexDirection: 'row', alignSelf: 'center' }}>
                                                          {(dataContent.fLocationImageJSON).map((x, index) => {
                                                              return (
                                                                  <View accessible={true} accessibilityLabel={'RestoProfileLocationImageView'} key={x.key} style={{ justifyContent: 'flex-start'  }}>
                                                        ...
                                                        <View style={{flexDirection: 'row', justifyContent: 'flex-start', width: 9*w/10}}>
                                                            <TouchableOpacity key={data.id} onPress={() => this.getUrlLocation(data.id)}>
                                                                <View style={{flexDirection: 'row', justifyContent: 'flex-start'}}>
                                                                    <Text preset={Text.preset.titleH2B} style={{color: '#7ED9FA'}}>{SGLocalize.translate('restoProfileScreen.LocDesc')}</Text>
                                                ...
                                                {data.floor.length !== 0 &&
                                                  ...
                                                  <View accessible={true} style={style.infoSectionDescContainer}>
                                                      <Text accessible={true} accessibilityLabel={'RestoProfileScreenPlaceName'} style={style.sectionDescText} preset={Text.preset.titleH3}>{data['fBuildingName' + this.Language.toUpperCase()]}</Text>
                                                      <Text accessible={true} accessibilityLabel={'RestoProfileScreenFloorLoc'} style={style.sectionDescText} preset={Text.preset.titleH3}>{this._constructFloorString(this.data.floor)}</Text>
                                                      <Text accessible={true} accessibilityLabel={'RestoProfileScreenLocHint'} style={style.sectionDescText} preset={Text.preset.titleH3}>{dataContent.fLocation}</Text>
                                                  </View>
                                              </View>
                                             
                                              <View accessible={true} style={style.infoSectionContainer}>
                                                  <Text accessible={true} accessibilityLabel={'RestoProfileScreenBookTitle'} style={style.infoSectionHeadingText} preset={Text.preset.titleH2B}>{SGLocalize.translate('restoProfileScreen.DescTitle')}</Text>
                                                  {this.noImageStory !== true &&
                                                  <ViewPager accessible={true} accessibilityLabel={'RestoProfileScreenBookViewPager'} style={style.sliderContainer} initialPage={0} showPageIndicator={true} transitionStyle={'scroll'} pageIndicatorStyle={{ position: 'absolute', bottom: 0 * p, flexDirection: 'row', alignSelf: 'center' }}>
                                                      {(dataContent.fImageSlider).map((x, index) => {
                                                          return (
                                                              <View accessible={true} accessibilityLabel={'RestoProfileScreenBookImageView'} key={x.key} style={{ justifyContent: 'flex-start' }}>
                                                              { x[this.imageSetting].uri !== SGHelperType.getSystemParamsValue('DefaultImage') &&
                                                                  <Image accessible={true} accessibilityLabel={'RestoProfileScreenBookImage'} style={style.sliderImage} source={{ uri: x[this.imageSetting].uri }}></Image>
                                                              }
                                                              </View>
                                                          )
                                                      })}
                                              ...
                                              {dataContent.fTypeDetail === 'longdescription' ?
                                                  (<Text accessible={true} accessibilityLabel={'RestoProfileScreenBookDescText'} style={style.descriptionText} preset={Text.preset.titleH3}>{dataContent.fLongDescription}</Text>)
                                                  :
                                                  (null)
                                              }
                              ...
                              <Animated.View accessible={true} style={[style.thumbnailContainer, { width: this._animIcon.interpolate({ inputRange: [0, 1], outputRange: [w * 0.375, w * 0.425] }), height: this._animIcon.interpolate({ inputRange: [0, 1], outputRange: [w * 0.375, w * 0.425] }), borderRadius: this._animIcon.interpolate({ inputRange: [0.25, 1], outputRange: [w * 0.25, 3 * p] }), borderWidth: this._animIcon.interpolate({ inputRange: [0, 0.35], outputRange: [3, 0] }), transform: [{ scale: this._animIcon.interpolate({ inputRange: [0, 1], outputRange: [0.5, 1] }) }, { translateX: this._animIcon.interpolate({ inputRange: [0, 1], outputRange: [w * 0.75, 0] }) }, { translateY: this._animIcon.interpolate({ inputRange: [0, 1], outputRange: [-w * 0.4, 0] }) }] }]}>
                                  <Image accessible={true} accessibilityLabel={'RestoProfileScreenLogoImage'} source={{ uri: dataContent.fStoreImageJSON[0][this.imageSetting].uri }} style={style.thumbnailImage}></Image>
                              </Animated.View>
                          </View>)
                          :
                          (null)
