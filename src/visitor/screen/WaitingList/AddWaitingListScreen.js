...
import { SGView as View, SGRootView as RootView, SGTouchableOpacity as TouchableOpacity, SGText as Text, SGIcon as Icon, SGScrollView as ScrollView, SGPopView, SGButton as Button, SGDialogBox, SGImage as Image,SGActivityIndicator as ActivityIndicator } from '../../../core/control';
...
export class AddWaitingListScreen extends SGBaseScreen {
    ...
    async _onRefreshData() {
        ...
        this.baseAddAPIParallel('getRestoHomeProfile', (async (v1) => { return VRestoProfileAPI.getRestoHomeProfile(v1) }).bind(this,this.restoKey), ((v) => {
            this.profileResto = v //await VRestoProfileAPI.getRestoHomeProfile(this.restoKey);
    ...
    async _onCancelButtonHandler(){
        this.waitingListData.fStoreKey = this.restoKey;
        try {
            ...
            SGDialogBox.showWarning(null, SGLocalize.translate("AlertMessage.Fail"), SGLocalize.translate("AlertMessage.FailAddWaitingList"), SGLocalize.translate("AlertMessage.OK"), () => { }, true)
        }
    }
    async _onAddButton() {
        this.paging = this.getPagingData()
        this.baseRunSingleAPIWithRedoOption('searchWaitingListVisitor', (async (v1,v2,v3) => { return tbVWaitingListAPI.searchWaitingListVisitor(v1,v2,v3) }).bind(this,this.filterCheckWaiting, [],this.paging), ((v) => {
            var checkDataWaiting = v //await tbVWaitingListAPI.searchWaitingListVisitor(this.filterCheckWaiting, [],this.paging);
    ...
    render() {
        ...
        return (
            ...
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'AddWaitingListScreenRootView'} style={style.mainView1}>
                ...
                <Image accessible={true} accessibilityLabel={'AddWaitingListScreenImage'} style={style.containerView1} source={{ uri: this.alreadyMount == true ? this.profileResto['fContent' + this._language.toUpperCase()].fStoreBackgroundImageJSON[0][this.imageSetting].uri : '' }}></Image>
