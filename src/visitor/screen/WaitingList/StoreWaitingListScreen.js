...
export class StoreWaitingListScreen extends SGBaseScreen {
    ...
    async componentDidMount() {
        ...
        this._unsubscribe = this.props.navigation.addListener('focus', async () => {
            ...
            this.interval = setInterval(async () => {
                await this._onSearchingItem();
                if (!this.animFlag) { setTimeout(() => { this.animFlag = true; this.baseCreateAnimation(500, null); }, 1000); }
            }, SGHelperType.getSysParamsValueToInt('StoreWaitingListScreenInterval1'));
            }
        });
    }
    ...
    async _onSearchingItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            this.baseAddAPIParallel('CheckUserCheckInHere', (async (v1) => { return tbVUserCheckInAPI.CheckUserCheckInHere(v1) }).bind(this,this.storeKey), ((v) => {
            ...
            this.baseAddAPIParallel('getStoreHomeHeader', (async (v1) => { return VStoreHomeAPI.getStoreHomeHeader(v1) }).bind(this,this.storeKey), ((v) => {
    ...
    _addWatingListPress() {
        if(this.canWaitingList)  SGHelperNavigation.navigatePush(this.props.navigation, "AddWaitingListStore", { callback: this._callBackForceUpdate.bind(this), storeKey: this.storeKey })
    ...
    _onCardPress(data) {
        ...
        SGHelperNavigation.navigatePush(this.props.navigation, "ViewWaitingListStore", { data: tempData, callback: this._callBackForceUpdate.bind(this), storeKey: this.storeKey, profileStore: this.profileStore, dataSetting: this.restoSettingsData })
    }

    async _refresh() {
        this.baseRunSingleAPIWithRedoOption('CheckUserCheckInHere', (async (v1) => { return tbVUserCheckInAPI.CheckUserCheckInHere(v1) }).bind(this,this.storeKey), ((v) => {
            this.isAlreadyCheckIn = v //await tbVUserCheckInAPI.CheckUserCheckInHere(this.storeKey);
            this.forceUpdate();
        }).bind(this), null,  SGHelperGlobalVar.getVar("ResponTimes"));
    }
    onHideReward() {
    ...
    async _SurpriseRewardCheckInStore() {
        try {
            if (!SGHelperGlobalVar.getVar('GlobalAnonymous')) {
                this.surpriseReward = await VRewardAPI.SurpriseRewardCheckInStore(this.storeKey)
    ...
    async _SurpriseRewardLikeTenant(storeKey) {
        try {
            if (!SGHelperGlobalVar.getVar('GlobalAnonymous')) {
                this.surpriseReward = await VRewardAPI.SurpriseRewardLikeStore(storeKey)
                if (this.surpriseReward.fID !== null) {
                    this.onShowReward()
                    this.forceUpdate()
                } else this.surpriseReward = { fID: null, fRewardID: { fTargetName: '', fShortDescription: '', fImageJSON: [] }, fRewardEN: { fTargetName: '', fShortDescription: '', fImageJSON: [] }, fRewardCN: { fTargetName: '', fShortDescription: '', fImageJSON: [] } }
            ...
            SGHelperErrorHandling.Handling(error,this._SurpriseRewardLikeTenant.bind(this,storeKey))
        }
    ...
    _checkCanAddWaitingList(){
        var data = this.profileStore.opHours
        console.log(data);
    ...
    render() {
        ...
        return (
            ...
            <RootView dummyStatusBar  accessible={true} accessibilityLabel={'RestoWaitingListScreenRootView'} style={style.mainView1}>
                ...
                {this.alreadyMount ?
               <View style={{flex:1}}>
                <StoreHomeNewHeader fullData={this.profileStore} noCheckButton animVar={this._baseAnimVar} accessible={true} accessibilityLabel={'StoreHomeScreenHeader'} isAlreadyCheckIn={this.isAlreadyCheckIn} currentUserData={this.currentUserData} language={this.Language} imageSetting={this.imageSetting} refresh={this._refresh.bind(this)} currentUserImage={this.currentUserImage} screen='StoreProfile' navigator={this.props.navigation} headerData={this.storeHeaderData} style={style.throwWHP} checkInReward={async () => await this._SurpriseRewardCheckInStore()} likeTenantGetReward={async () => await this._SurpriseRewardLikeTenant( this.storeKey)}></StoreHomeNewHeader>
