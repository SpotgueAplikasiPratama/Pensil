...
import { ClothToBuyCard } from '../../container_V2/ClothToBuyCard';
...
export class ClothToBuyResultScreen extends SGBaseScreen {
    ...
    createStyleSheet = (whp) => {
        ...
        return StyleSheet.create({
            ...
            textView1: { width: w, height: w * 0.12, alignItems: 'flex-start', },
            text1: { color: '#7a7a7a' }
        });
    }
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        this.placeID = this.props.route.params.placeID;

        if(resetPaging){
            this.setState({ refresh: true })
            this.pagingCounter= 0
            this.paging = this.getPagingData()
            this.baseRunSingleAPIWithRedoOption('getClothToBuyResult', (async (v1,v2,v3,v4) => { return VAliceResultAPI.getClothToBuyResult(v1,v2,v3,v4) }).bind(this, this.props.route.params.userAnswerData, this._filterData, this._sortData,this.paging), (async (v) => {
                this.renderData = v //await VAliceResultAPI.getClothToBuyResult(this.props.route.params.userAnswerData, this._filterData, this._sortData,this.paging);
    ...
    async _onRefresh() {
        ...
        if(!this.refresh && !this.state.loading){
            ...
            this.baseRunSingleAPIWithRedoOption('getClothToBuyResult', (async (v1,v2,v3,v4) => { return VAliceResultAPI.getClothToBuyResult(v1,v2,v3,v4) }).bind(this, this.props.route.params.userAnswerData, this._filterData, this._sortData,this.paging), ((v) => {
            this.renderData = v
            this.pagingCounter = this.renderData.length
            if(v.length<SGHelperType.getPaging())this.setState({stopPulling:true})
            else this.setState({stopPulling:false})
    ...
    async _onLoad(){
        ...
        if(!this.state.loading && !this.state.stopPulling){    
            ...
            this.paging = this.getPagingData()
            this.baseRunSingleAPIWithRedoOption('getClothToBuyResult', (async (v1,v2,v3,v4) => { return VAliceResultAPI.getClothToBuyResult(v1,v2,v3,v4) }).bind(this,this.props.route.params.userAnswerData, this._filterData, this._sortData,this.paging), (async (v) => {
                var resData = v //await VAliceResultAPI.getClothToBuyResult(this.props.route.params.userAnswerData, this._filterData, this._sortData,this.paging);
                if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        this.renderData = [];
        this._filterData = filterDAO.getClothToBuyFilterData();
        this._sortData = sortDAO.getClothToBuySortData(this.Language.toUpperCase());
        this.currentUser = SGHelperGlobalVar.getVar('GlobalCurrentUser');
        this.userData = SGHelperGlobalVar.getVar('GlobalCurrentUserData');
        this.currentUserImage = SGHelperGlobalVar.getVar('GlobalCurrentUserImage');
        this.imageSetting = SGHelperGlobalVar.getVar('GlobalImageSetting');
    ...
    render() {
        ...
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'ClothToBuyResultScreenRootView'} style={style.mainContainer}>
                <RibbonHeader borderTopOff imageSetting={this.imageSetting} title={SGLocalize.translate("ClothToBuyResultScreen.screenTitle")} ribbonWidth={0.45}></RibbonHeader>
                {
                    this.alreadyMount ?
                        this.renderData.length !== 0 ?
                            <FlatList refreshing={this.state.refreshFlatList} onRefresh={this._onRefresh.bind(this)} accessible={true} accessibilityLabel={'ClothToBuyResultScreenList'} showsVerticalScrollIndicator={false} contentContainerStyle={{ justifyContent: 'flex-start', alignItems: 'center' }} data={this.renderData} renderItem={({ item }) => {
                                return (
                                    <ClothToBuyCard onLike={((item, s,c)=>{ item.fUserLikedThis = s; item.fLikeCountRec+=c; this.forceUpdate(); }).bind(this,item)} accessible={true} accessibilityLabel={'ClothToBuyResultScreenCTBCard'} commentPackage={this._getCommentResource(item)} likePackage={this._getLikeResource(item)} key={item.key} language={this.Language} data={item} currency={this.currentUserCurrency} navigator={this.props.navigation} imageSetting={this.imageSetting} likeText={SGLocalize.translate("ClothToBuyResultScreen.likeText")} contentKey={item.key} style={style.containerView1}></ClothToBuyCard>
                                );
                ...
                <Animated.View style={{ position: 'absolute', transform: [{ translateY: this._baseAnimY.interpolate({ inputRange: [-1, 0], outputRange: [(SGHelperWindow.getStatusBarHeight() - SGHelperWindow.getHeaderHeight()), SGHelperWindow.getStatusBarHeight()] }) },], height: SGHelperWindow.getHeaderHeight(), width: '100%' }}>
                    ...
                    <FullSearchBar noSearchBar noIconSearchBar accessible={true} accessibilityLabel={'ClothToBuyResultScreenSearchBar'} imageSetting={this.imageSetting} sortData={this._sortData} onApplySort={ this._setSort.bind(this) }  onPressFilter={() => { SGHelperNavigation.navigatePush(this.props.navigation, 'Filter', { filterData: this._filterData, buildingMatrix: this._buildingMatrix, onApplyFilter: (v) => { this._setFilter(v) }, onCloseFilter: (v) => { console.log(v) } }) }} navigator={this.props.navigation} style={this.style.throwWHP}></FullSearchBar>
                </Animated.View>
