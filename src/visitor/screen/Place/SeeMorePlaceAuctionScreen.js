...
import { VSeeMorePlaceAuctionAPI } from '../../api/VSeeMorePlaceAuctionAPI';
import { EmptyDetailView } from '../../container_V2/EmptyDetailView';

export class SeeMorePlaceAuctionScreen extends SGBaseScreen {
    ...
    async _onRefreshAllItem(resetPaging = false) {
        // try {
            ...
            if(resetPaging){
                ...
                this.baseRunSingleAPIWithRedoOption('getPlaceAuctionList', (async (v1, v2, v3, v4) => { return VSeeMorePlaceAuctionAPI.getPlaceAuctionList(v1, v2, v3, v4) }).bind(this, this.props.route.params.contentKey, this._filterData, this._sortData,this.paging), ((v) => {           
                    this.data = v//await VSeeMorePlaceAuctionAPI.getPlaceAuctionList(this.props.route.params.contentKey, this._filterData, this._sortData,this.paging);
    ...
    async _onLoad(){
        ...
        // try {
            ...
            if(!this.state.loading && !this.state.stopPulling){
                ...
                this.baseRunSingleAPIWithRedoOption('getPlaceAuctionList', (async (v1, v2, v3, v4) => { return VSeeMorePlaceAuctionAPI.getPlaceAuctionList(v1, v2, v3, v4) }).bind(this, this.props.route.params.contentKey, this._filterData, this._sortData,this.paging), ((v) => {           
                    var resData = v// VSeeMorePlaceAuctionAPI.getPlaceAuctionList(this.props.route.params.contentKey, this._filterData, this._sortData,this.paging);
                    if(resData.length!==0){
    ...
    constructor(props, context, ...args) {
        ...
        this.searchKeyword = '';
        this.imageSetting = SGHelperGlobalVar.getVar('GlobalImageSetting');
        this._filterData = filterDAO.getPlaceAuctionFilterData();
        this._sortData = sortDAO.getAuctionPlaceSortData(this.Language.toUpperCase());
        this._buildingMatrix = filterDAO.getBuildingMatrix();
        this.currentUserImage = SGHelperGlobalVar.getVar('GlobalCurrentUserImage');
    ...
    render() {
        ...
        return (
            <RootView dummyStatusBar dummyHeaderBar accessible={true} accessibilityLabel={'SeeMorePlaceAuctionScreenRootView'} style={style.mainView1}>
                <RibbonHeader imageSetting={this.imageSetting} title={SGLocalize.translate("SeeMorePlaceAuctionScreen.screenTitle", { placeName: this.contentData['fContent' + this.Language.toUpperCase()].fBuildingName })}></RibbonHeader>
                {this.alreadyMount ?
                    this.data.length !== 0 ?
                        <FlatList refreshing={this.state.refreshFlatList} onRefresh={this._onRefresh.bind(this)} accessible={true} accessibilityLabel={'SeeMorePlaceAuctionScreenList'} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingVertical: p, justifyContent: 'flex-start', alignItems: 'center' }} data={this.data} renderItem={({ item }) => {
                            return (
