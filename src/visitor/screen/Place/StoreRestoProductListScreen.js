...
export class StoreRestoProductListScreen extends SGBaseScreen {
    ...
    async componentDidMount() {
        ...
        var jsonInput = { fID: '', fContentType: 'SeeAllDiscountSales',  fTargetKey: SGHelperType.getGUID(), fUserKey: '', fActive: 'Y', fCreatedByID: '', fCreatedDate: new Date(), fLastModifiedByID: '', fLastModifiedDate: new Date() }
    ...
    async _onRefreshAllItem(resetPaging = false) {
        ...
        if(resetPaging){
            ...
            if(this.searchKeyword === ''){
                this.baseRunSingleAPIWithRedoOption('SearchBuildingHighlightSeeMoreSaleDiscount', (async (v1,v2,v3,v4) => { return VMallHomeAPI.SearchBuildingHighlightSeeMoreSaleDiscount(v1, v2, v3, v4) }).bind(this, this.props.route.params.buildingKey, this._filterData, this.sortArr,this.paging), ((v) => {
                    this.data = v 
                    this._onPullAPIDone()
                }).bind(this), null,  SGHelperGlobalVar.getVar("ResponTimes"));
            }else{
                this.baseRunSingleAPIWithRedoOption('SearchBuildingHighlightSeeMoreSaleDiscountWithKeyword', (async (v1,v2,v3,v4,v5,v6) => { return VMallHomeAPI.SearchBuildingHighlightSeeMoreSaleDiscountWithKeyword(v1, v2, v3, v4,v5,v6) }).bind(this, this.props.route.params.buildingKey,this.Language,this.searchKeyword, this._filterData, this.sortArr,this.paging), ((v) => {
                    this.data = v 
                    this._onPullAPIDone()
                }).bind(this), null,  SGHelperGlobalVar.getVar("ResponTimes"));
            }
    ...
    render() {
       ...
       console.log('StoreRestoProductListScreen');
        return (
                ...
                {this.alreadyMount ?
                        <View style={{ flex: 1 }}>
                            <View accessible={true} accessibilityLabel={'RestoMenuListScreenHeaderView'} style={style.header}>
                                <Text accessible={true} accessibilityLabel={'RestoMenuListScreenTitleCategory'} preset={Text.preset.titleH1B} style={style.textTitle}>{SGLocalize.translate('mallHomeScreen.SecretSaleList')}</Text>
