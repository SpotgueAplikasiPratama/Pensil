...
export class DetailTipsAndTrickScreen extends SGBaseScreen {
    createStyleSheet = (whp) => {
        var { w, h, p } = whp;
        return StyleSheet.create({
            v1: { width: w, height: h, backgroundColor: "white", justifyContent: 'flex-start' },
            v2: { position:'absolute',bottom:10,flexDirection:'row',justifyContent:'space-around',backgroundColor:'transparent'},
            v2_1: { width: (w - 2 * p) * 0.3, height: w * 0.1, borderWidth: 3, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', borderRadius: w * 0.02, borderColor: "#e9e9e9", marginRight:2*p,backgroundColor:"white"},
            v2_2: { width: (w - 2 * p) * 0.3, height: w * 0.1, borderWidth: 3, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', borderRadius: w * 0.02, borderColor: "blue", marginLeft:2*p,backgroundColor:"white" },
        });
    }
    ...
    async _refreshData() {
        // try {
            ...
            // this.directoryDataSpotgue = await tbCDirectoryDataSpotgueAPI.getDirectoryDataSpotgueByID(this.fID)
            // this.data = await tbCActivityDataSpotgueAPI.getActivityDataSpotgueByID(this.fID)
    ...
    render() {
        ...
        return (
            <RootView dummyStatusBar accessible={true} accessibilityLabel={'DetailTipsAndTrickScreenRootView'} style={style.v1}>
