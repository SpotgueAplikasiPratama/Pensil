...
export class TipsAndTrickScreen extends SGBaseScreen {

    getDataFilter(lang) {
        return ([
            { name: 'fVisitor', operator: '=', value: 'Y', visible: false },
            { name: 'fType', operator: '=', value: 'tips', visible: false },
            { name: 'fSearchKey' + lang, operator: 'CONTAINS', value: null, type: 'search', title: "Search for tips and trick", visible: true },
    ...
    _onPress(fID) {
        SGHelperNavigation.navigatePush(this.props.navigation, "DetailTipsAndTrickScreen", { fID: fID, language: this._language });
    }
    ...
    render() {
        ...
        return (
            <RootView dummyStatusBar accessible={true} accessibilityLabel={'TipsAndTrickScreenRootView'} style={style.v1}>
                ...
                <View style={style.v3}>
                    <Text accessible={true} accessibilityLabel={'TipsAndTrickScreen'} preset={Text.preset.titleH1B} style={style.text}>{SGLocalize.translate("ChooseScreenProfile.HeaderTitleTipsAndTrick")}</Text>
                </View>
                <SGFormSearchFilterSort accessible={true} accessibilityLabel={'TipsAndTrickScreenFormSearchFilterSort'} filterData={this.dataFilter} sortData={this.dataSorting} onValueChange={(v) => { this._onRefresh(); }} />
                <Text accessible={true} accessibilityLabel={'TipsAndTrickScreenTitleText'} style={style.text1} preset={Text.preset.titleH3}>{SGLocalize.translate("ChooseScreenProfile.TextTitleTipsAndTrick1")}</Text>
                <FlatList accessible={true} accessibilityLabel={'TipsAndTrickScreenList'} contentContainerStyle={{ justifyContent: 'flex-start', alignItems: 'center' }} data={this._data} renderItem={({ item }) => {
                    return (
                        <TouchableOpacity onPress={this._onPress.bind(this, item)}>
                            <View accessible={true} accessibilityLabel={'TipsAndTrickScreenListView'} style={style.v2} >
                                <View accessible={true} accessibilityLabel={'TipsAndTrickScreenTextView'} style={style.v2_1}><Text accessible={true} accessibilityLabel={'TipsAndTrickScreenItemText'} preset={Text.preset.titleH3}>{item["fTitle" + language]}</Text></View>
                                <View accessible={true} accessibilityLabel={'TipsAndTrickScreenIconView'} style={style.v2_2}><Icon accessible={true} accessibilityLabel={'TipsAndTrickScreenSquareIcon'} name={Icon.Icon.rightSquare} preset={Icon.preset.h5}></Icon></View>
                            </View>
                        </TouchableOpacity>
                    );
