...
import { RestoCardSmall } from './RestoCardSmall';
import { SGHelperNavigation , SGHelperGlobalVar} from '../../core/helper';
import image from '../asset/image';

export class RestoSlider extends SGBaseContainer {
    ...
    render() {
        console.log('RestoSlider-render')
        var { w, h, p } = this.whp;
        ...
        return (
            <View accessible={true} style={style.bigContainer}>
                <View accessible={true} accessibilityLabel={'RestoSliderRootView'} style={style.mainContainer}>
                    {this.titleBar ?
                        (<View accessible={true} accessibilityLabel={'RestoSliderHeaderView'} style={style.heading}>
                            <Text accessible={true} accessibilityLabel={'RestoSliderTitle'} preset={Text.preset.titleH2B} style={style.headingText}>{this.props.title}</Text>
                            <TouchableOpacity onPress={() => SGHelperNavigation.navigatePush(this.props.navigator, this.props.seeMoreScreen, { title: this.props.seeMoreTitle, contentKey: this.props.contentKey, contentData: this.props.contentData })}>
                                <Text accessible={true} accessibilityLabel={'RestoSliderSeeMore'} preset={Text.preset.titleH4} style={style.seeMoreText}>{this.props.seeMoreLabel}</Text>
                            </TouchableOpacity>
                        </View>)
                    :
                        (null)
                    }
                    <ScrollView accessible={true} accessibilityLabel={'RestoSliderScrollView'} showsHorizontalScrollIndicator={false} style={style.sliderContainer} contentContainerStyle={{ justifyContent: 'center', alignItems: 'center' }} horizontal>
                        {
                            data.map((x, index) => {
                                return (
                                    <RestoCardSmall accessible={true} accessibilityLabel={'RestoSliderRestoCardSmall'} key={x.key} contentKey={x.key} screen={this.props.screen} navigator={this.props.navigator} restoName={x['fContent' + (this.props.language).toUpperCase()].fStoreName} placeName={x['fBuildingName' + (this.props.language).toUpperCase()]} image={x['fContent' + (this.props.language).toUpperCase()]['fStoreImageJSON'][0][this.props.imageSetting].uri} index={index} style={style.cardSlider}>
                                    </RestoCardSmall>
                                )
                            })
                        }
