...
export class InstagramSpotSlider extends SGBaseContainer {
    createStyleSheet = (whp) => {
        ...
        return StyleSheet.create({
            ...
            headingText: {maxWidth: this.props.seeMore? w * 0.75:w*0.96, color: '#000000', marginVertical: 0, alignSelf: 'center'},
            ...
            textView1: { backgroundColor: 'rgba(255,255,255,0.5)', width: w},
    ...
    render() {
        ...
        return (
            this.noImage !== true &&
            <View accessible={true} accessibilityLabel={'InstagramSpotSliderRootView'} style={style.mainContainer} >
                <View accessible={true} style={style.header}>
                    <View accessible={true} accessibilityLabel={'InstagramSpotSliderHeaderView'} style={style.heading}>
                        <Text accessible={true} preset={Text.preset.titleH2B} style={style.headingText}>{this.props.titleHeading}</Text>
                        {this.seeMore ?
                            (<TouchableOpacity>
                                <Text accessible={true} preset={Text.preset.titleH4} style={style.seeMoreHeadingText}>{this.props.seeMoreLabel}</Text>
                            </TouchableOpacity>)
                        :
                            (null)
                        }
                    ...
                    <ViewPager accessible={true} accessibilityLabel={'InstagramSpotSliderViewPager'} style={style.sliderContainer} initialPage={0} showPageIndicator={true} transitionStyle={'scroll'} pageIndicatorStyle={{position: 'absolute', bottom: 0, marginVertical: 0, flexDirection: 'row', alignSelf: 'center' }}>
                    {
                        data.map((x) => {
                            console.log(x);
                            ...
                            return (
                                <TouchableOpacity key={x.key}>
                                    <Image accessible={true} accessibilityLabel={'HighlightSummaryImage'} style={{...style.sliderImage,...img}} source={{ uri: x[this.props.imageSetting].uri }}>
                                        {x.text !== '' ?
                                            // (<View accessible={true} accessibilityLabel={'HighlightSummaryDescView'} style={[style.textView1],{...txt}}>
