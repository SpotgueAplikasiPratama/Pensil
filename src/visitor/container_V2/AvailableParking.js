...
export class AvailableParking extends SGBaseContainer {
    createStyleSheet = (whp) => {
        var { w, h, p } = whp;
        return StyleSheet.create({
            header: { width: w, paddingLeft: p * 4, paddingRight: p * 4, flexDirection: 'row', justifyContent: 'space-between', backgroundColor: '#FFFFFF', paddingVertical: p,marginBottom:p },
    ...
    render() {
        ...
        return (
               ...
               <View accessible={true} accessibilityLabel={'ParkingHighlightTopTextView'} style={style.header}>
                    <Text accessible={true} accessibilityLabel={'ParkingHighlightTitleText'} style={{ color: '#000000' }} preset={Text.preset.titleH4B}>{SGLocalize.translate("parkingHighlight.title")}</Text>
                    <Text accessible={true} accessibilityLabel={'ParkingHighlightTitleText'} style={style.textTitle} preset={Text.preset.titleH5}>{tR("AvailableParkir.LastUpdated"+this.timeString, { count: this.timeDifference })}</Text>
                </View>
