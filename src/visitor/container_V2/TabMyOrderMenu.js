...
import { MyOrderMenuActiveCard } from '../container_V2/MyOrderMenuActiveCard';
import { MyOrderMenuHistoryCard } from '../container_V2/MyOrderMenuHistoryCard';
...
export class TabMyOrderMenu extends SGBaseContainer {
    ...
    _onCardActivePress(data) {
        console.log(data);
        SGHelperNavigation.navigatePush(this.props.navigator, 'OrderList', { fID: data.ordermenufID, restoKey: data.fStoreKey })
        // SGHelperNavigation.navigatePush(this.props.navigator, 'RestoOrderMenuDetailScreen', { fID: data.ordermenufID, restoKey: data.fStoreKey, active: true, callback: this.props.callback });
    }

    _onCardHistoryPress(data) {
        console.log(data);
        SGHelperNavigation.navigatePush(this.props.navigator, 'RestoOrderMenuDetailScreen', { fID: data.ordermenufID, restoKey: data.fStoreKey, active: false, callback: this.props.callback });
    }
                ...
                
            {
                ...
                <View> 
                    <FlatList refreshing={this.state.refreshFlatListData} onRefresh={refreshData} accessible={true} accessibilityLabel={'FlatListOrderMenu'} data={data}  contentContainerStyle={{ justifyContent: 'flex-start', alignItems: 'center' }}  renderItem={({ item, index }) => {
                        return (
                            <MyOrderMenuActiveCard key={item.ordermenufID} accessible={true} accessibilityLabel={'TabMyOrderMenuMyOrderActiveCard'} imageSetting={this.imageSetting} screen='OrderList' navigator={this.props.navigator} data={item} language={this.props.language} userData={this.props.userData} style={this.style.containerView1} onCardPress={this._onCardActivePress.bind(this, item)}   >
                            </MyOrderMenuActiveCard>
                ...
                
            {
                ...
                <View> 
                    <FlatList refreshing={this.state.refreshFlatListHistory} onRefresh={refreshDataHistory} accessible={true} accessibilityLabel={'FlatListRewardHistory'} data={data}  contentContainerStyle={{ justifyContent: 'flex-start', alignItems: 'center' }}  renderItem={({ item, index }) => {
                        return (
                            <MyOrderMenuHistoryCard key={item.ordermenufID} accessible={true} accessibilityLabel={'TabMyOrderMenuMyOrderHistoryCard'} imageSetting={this.imageSetting} currency={this.currentUserCurrency} screen='MyOrderMenuHistory' navigator={this.props.navigator} style={this.style.containerView1} data={item} language={this.props.language} userData={this.props.userData} onHistoryPress={this._onCardHistoryPress.bind(this, item)}>
                            </MyOrderMenuHistoryCard>
    ...
    render() {
        var { w, h, p } = this.whp;
        var style = this.style;
        var dataActive = this.props.orderMenuActiveData;
        var dataHistory = this.props.orderMenuHistoryData;
