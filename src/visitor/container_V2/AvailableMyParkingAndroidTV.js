...
export class AvailableMyParkingAndroidTV extends SGBaseContainer {
    createStyleSheet = (whp) => {
        var { w, h, p } = whp;
        return StyleSheet.create({
            header: { flexDirection:'row', width: w, paddingLeft: p * 4, paddingRight: p * 10, justifyContent: 'space-between', alignItems:'flex-start', backgroundColor: '#FFFFFF', paddingVertical: p },
    ...
    render() {
        ...
        return (
            ...
            <View accessible={true} accessibilityLabel={'ParkingHighlightTopTextView'} style={style.header}>
                <Text accessible={true} accessibilityLabel={'ParkingHighlightTitleText'} style={{ color: '#000000' }} preset={Text.preset.heading7B}>{SGLocalize.translate("parkingHighlight.title")}</Text>
                <View style={{width:w*0.435,alignItems:'flex-end',marginTop:2*p}}>
                    <Text accessible={true} accessibilityLabel={'ParkingHighlightTitleText'} style={{color:'#A2A2A2'}} preset={Text.preset.heading10}>{tR("AvailableParkir.LastUpdated"+this.timeString, { count: this.timeDifference })}</Text>
                </View>
