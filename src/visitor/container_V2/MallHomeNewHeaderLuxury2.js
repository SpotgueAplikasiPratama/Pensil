...
import { SGHelperNavigation, SGHelperGlobalVar, SGHelperType, SGHelperStyle, SGHelperWindow,SGHelperErrorHandling } from '../../core/helper';
...
export class MallHomeNewHeaderLuxury2 extends SGBaseContainer {
    createStyleSheet = (whp) => {
        var { w, h, p } = whp;
        return StyleSheet.create({
            mainContainer: { width: w, flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start' },
            imageHeaderContainer: {overflow:'hidden', width: w, backgroundColor: '#FFFFFF', marginTop: p * -1, justifyContent: 'flex-start', alignItems: 'center' },
            placeCoverImage: { width: w, height: w * 0.49, resizeMode: 'cover', borderRadius: 0, padding:0 },
            overlay: { width: w, height: '100%', backgroundColor: 'transparent', justifyContent:'flex-start',padding:0},
        ...
        return (
            <View>
                ...
                <View accessible={true} style={style.mainContainer}>
                    <Animated.View style={{marginTop: this._animW.interpolate({ inputRange: [0, 1], outputRange: [0,0] })}}>
                        <Animated.View accessible={true} style={[style.imageHeaderContainer, { height: this._animW.interpolate({ inputRange: [0, 1], outputRange: [w * 0.25 + SGHelperWindow.getStatusBarHeight()+SGHelperWindow.getHeaderHeight()-11*p, w * 0.25 + SGHelperWindow.getStatusBarHeight()+SGHelperWindow.getHeaderHeight()] }) }]}>
                            <Image accessible={true} accessibilityLabel={'MallHomeHeaderRootImageBackground'} style={style.placeCoverImage} source={{ uri: dataContent.fImageBackgroundJSON[0][this.props.imageSetting].uri }}>
                                <View accessible={true} accessibilityLabel={'MallHomeHeaderOverlayView'} style={style.overlay}>
                                    <Image source={require('../../core/asset/image/mall_top.png')} resizeMode={'stretch'} style={{ marginLeft:p, marginTop:-p, backgroundColor:'transparent',borderRadius:0, width:w, height:'100%', }}></Image>
                                </View>
                            </Image>
                        </Animated.View>
                    </Animated.View>
                    <Animated.View accessible={true} style={[style.placeDetailContainer,{paddingTop:this._animW.interpolate({ inputRange: [0, 1], outputRange: [data.lastVisited !== null ?9*p:4*p,12*p] }),marginBottom:this._animW.interpolate({ inputRange: [0, 1], outputRange: [-3*p,0] }) }]}>
                        <View accessible={true} style={style.leftContainer}>
                            <Animated.View style={{width:this._animW.interpolate({ inputRange: [0, 1], outputRange: [0, w-8*p] }), }}>
                                <Text accessible={true} accessibilityLabel={'MallHomeHeaderBottomDataText'} preset={Text.preset.titleH2B} style={style.placeNameText} numberOfLines={1} >{(dataContent.fBuildingName).toUpperCase()}</Text>
                            </Animated.View>
                            <Animated.View style={{width:this._animW.interpolate({ inputRange: [0, 1], outputRange: [0, w-8*p] }), }}>
                    ...
                    <Animated.View accessible={true} style={[style.checkInOutBtnContainer,{marginRight:this._animW.interpolate({ inputRange: [0, 1], outputRange: [-7*p, 0] }), top:this._animW.interpolate({ inputRange: [0, 1], outputRange: [w * 0.425-14*p+ SGHelperWindow.getStatusBarHeight(), w * 0.425+ SGHelperWindow.getStatusBarHeight()] })} , { transform: [{scale:this._animW.interpolate({ inputRange: [0, 1], outputRange: [0.85,1] })},{ translateY: SGHelperType.isDefined(this.props.animVar) ? this.props.animVar.interpolate({ inputRange: [0, 1], outputRange: [0, -w * 0.23] }) : 0 }] }]}>
                    ...
                    <Animated.View accessible={true} style={[style.thumbnailContainer, {marginLeft:this._animW.interpolate({ inputRange: [0, 1], outputRange: [1*p, 4*p] }), top:this._animW.interpolate({ inputRange: [0, 1], outputRange: [w * 0.26 + 11 * p-6*p+ SGHelperWindow.getStatusBarHeight(), w * 0.26 + 11 * p+ SGHelperWindow.getStatusBarHeight()] })} , { transform: [{scale: this._animW.interpolate({ inputRange: [0, 1], outputRange: [0.85,1] })},{ translateY: SGHelperType.isDefined(this.props.animVar) ? this.props.animVar.interpolate({ inputRange: [0, 1], outputRange: [0, -w * 0.2] }) : 0 }] }]}>
