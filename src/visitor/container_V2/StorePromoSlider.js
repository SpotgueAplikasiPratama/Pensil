...
export class StorePromoSlider extends SGBaseContainer {
    ...
    _onSeeMorePress(){
        ...
        if(this.props.befSeeMoreScreen == 'HomeFav'){
            var jsonInput = { fID: '', fContentType: 'SeeAllFavoriteStoreEvent',  fTargetKey: SGHelperType.getGUID(), fUserKey: '', fActive: 'Y', fCreatedByID: '', fCreatedDate: new Date(), fLastModifiedByID: '', fLastModifiedDate: new Date() }
            this._addUserClick(jsonInput)
        }else if(this.props.befSeeMoreScreen == 'HomeTren'){
            var jsonInput = { fID: '', fContentType: 'SeeAllTrendingStoreEvent',  fTargetKey: SGHelperType.getGUID(), fUserKey: '', fActive: 'Y', fCreatedByID: '', fCreatedDate: new Date(), fLastModifiedByID: '', fLastModifiedDate: new Date() }
            this._addUserClick(jsonInput)
        }else if(this.props.befSeeMoreScreen == 'MallHighlight'){
            var jsonInput = { fID: '', fContentType: 'SeeAllMallHighlightStoreEvent',  fTargetKey: SGHelperType.getGUID(), fUserKey: '', fActive: 'Y', fCreatedByID: '', fCreatedDate: new Date(), fLastModifiedByID: '', fLastModifiedDate: new Date() }
            this._addUserClick(jsonInput)
        }else if(this.props.befSeeMoreScreen == 'StoreHighlight'){
            var jsonInput = { fID: '', fContentType: 'SeeAllStoreHighlightStoreEvent',  fTargetKey: SGHelperType.getGUID(), fUserKey: '', fActive: 'Y', fCreatedByID: '', fCreatedDate: new Date(), fLastModifiedByID: '', fLastModifiedDate: new Date() }
            this._addUserClick(jsonInput)
        }
        ...
        return (
            <View accessible={true} accessibilityLabel={'StorePromoMainContainer'} style={style.mainContainer}>
                <View accessible={true} accessibilityLabel={'StorePromoHeader'} style={style.header}>
                    <View accessible={true} accessibilityLabel={'StorePromoHeading'} style={style.heading}>
                        <Text accessible={true} preset={Text.preset.titleH2B} style={style.headingText}>{SGLocalize.translate('HomeScreen.WhatsHappeningStoreTitle')}</Text>
                        {this.oneLineSeeMore ?
                            (<TouchableOpacity onPress={() => this._onSeeMorePress()}>
                                <Text accessible={true} accessibilityLabel={'StorePromoSliderSeeMore'} preset={Text.preset.titleH4} style={style.seeMoreHeadingText}>{this.props.seeMoreLabel}</Text>
                            </TouchableOpacity>)
                        :
                            (null)
                        }
                    ...
                    {this.twoLine ?
                        (<View accessible={true} accessibilityLabel={'StorePromoDescription'} style={style.description}>
                            <Text accessible={true} preset={Text.preset.titleH3} style={style.descriptionText}>{this.props.title}</Text>
                            <TouchableOpacity onPress={() => this._onSeeMorePress()}>
                                <Text accessible={true} accessibilityLabel={'StorePromoSliderSeeMore'} preset={Text.preset.titleH4B} style={style.seeMoreDescriptionText}>{this.props.seeMoreLabel}</Text>
                            </TouchableOpacity>
                        </View>)
                    :
                        (null)
                ...
                <ViewPager accessible={true} accessibilityLabel={'StorePromoSliderViewPager'} style={style.sliderContainer} initialPage={0} showPageIndicator={true} transitionStyle={'scroll'} pageIndicatorStyle={{ position: 'absolute', bottom: 0 * p, flexDirection: 'row', alignSelf: 'center' }}>
                    {
                        data.map((x) => {
                            return (
                                <TouchableOpacity key={x.key} onPress={() => { SGHelperNavigation.navigatePush(this.props.navigator, 'StorePromoDetail', { contentKey: x.key }) }}>
